param(
  [string]$Model = "",
  [string]$LlamacppBaseUrl = "http://localhost:8080/v1",
  [string]$ProxyAddress = "",
  [switch]$SkipDirectLlamacppPreflight,
...
$tmpDir = Join-Path ([System.IO.Path]::GetTempPath()) ("lip-llamacpp-smoke-" + [System.Guid]::NewGuid().ToString("N"))
...
function Read-FirstLlamacppModel {
  $modelsURL = ($LlamacppBaseUrl.TrimEnd("/")) + "/models"
  $models = Invoke-RestMethod -Method Get -Uri $modelsURL -TimeoutSec 10
  if (-not $models.data -or $models.data.Count -eq 0) {
    throw "llama.cpp returned no models from $modelsURL"
  }
...
function Invoke-TextChat {
  ...
  $body = @{
    ...
    max_tokens = 64
    messages = @(
      @{ role = "user"; content = "/no_think Reply with exactly: LIP LLAMACPP TEXT SMOKE OK" }
    )
...
if ([string]::IsNullOrWhiteSpace($Model)) {
  $Model = Read-FirstLlamacppModel
}
if ([string]::IsNullOrWhiteSpace($ProxyAddress)) {
  $ProxyAddress = New-FreeLoopbackAddress
}

if (-not $SkipDirectLlamacppPreflight) {
  try {
    $direct = Invoke-TextChat -Uri (($LlamacppBaseUrl.TrimEnd("/")) + "/chat/completions") -RequestModel $Model
    $directText = [string]$direct.choices[0].message.content
    if ([string]::IsNullOrWhiteSpace($directText)) {
      throw "direct llama.cpp returned an empty assistant message"
    }
    Write-Host "llamacpp direct text preflight: PASS"
  } catch {
    throw "direct llama.cpp text preflight failed for model '$Model': $($_.Exception.Message)"
  }
}

$defaultRoute = "llamacpp:$Model"
$proxyRequestModel = "llamacpp:$Model"
...
try {
  ...
  if ($text -notmatch "(?i)LIP|LLAMACPP|SMOKE|OK") {
    throw "unexpected assistant response: $text"
  }

  Write-Host "llamacpp text smoke: PASS"

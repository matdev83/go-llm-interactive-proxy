...
func validateInvocationWire(inv Invocation) error {
	...
	call, err := invocationToCanonicalCall(inv)
	if err != nil {
		return err
	}
...
func invocationToCanonicalCall(inv Invocation) (lipapi.Call, error) {
	...
	call := lipapi.Call{
		ID:         inv.RequestID,
		Session:    lipapi.SessionRef{ALegID: inv.ALegID},
	...
	if inv.ItemAuthority {
		...
		for _, m := range inv.Instructions {
			msg, err := messageFromInvocationDTO(m)
			if err != nil {
				return lipapi.Call{}, err
			}
			call.Instructions = append(call.Instructions, msg)
		}
...
func messageFromInvocationDTO(m Message) (lipapi.Message, error) {
	parts, err := partsFromInvocationDTO(m.Parts)
	if err != nil {
		return lipapi.Message{}, err
	}
	return lipapi.Message{Role: m.Role, Parts: parts}, nil
}

func partsFromInvocationDTO(in []Part) ([]lipapi.Part, error) {
	out := make([]lipapi.Part, 0, len(in))
	for i, p := range in {
		part, err := partFromInvocationDTO(p, fmt.Sprintf("Parts[%d]", i))
		if err != nil {
			return nil, err
		}
		out = append(out, part)
...
func partFromInvocationDTO(p Part, field string) (lipapi.Part, error) {
	switch p.Kind {
	case PartKindText:
		t := ""
		if p.Text != nil {
	...
	case PartKindImageRef:
		...
		return lipapi.Part{Kind: lipapi.PartImageRef, ImageRef: r}, nil
	case PartKindFileRef:
		r := ""
		if p.FileRef != nil {
			r = *p.FileRef
		}
		return lipapi.Part{Kind: lipapi.PartFileRef, FileRef: r}, nil
	...
	case PartKindJSON:
		...
		if len(b) == 0 {
			return lipapi.Part{}, fmt.Errorf("%w: %s json part requires content", ErrInvalidInvocation, field)
		}
		return lipapi.Part{Kind: lipapi.PartJSON, Content: b}, nil
	default:
		return lipapi.Part{}, fmt.Errorf("%w: %q", ErrUnsupportedPartKind, p.Kind)
	}
}

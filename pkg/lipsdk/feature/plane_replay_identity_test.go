...
// TestOrderedIdentityPlanes_MapBacked_MalformedStateAndRollback tests map-backed storage for:
// A. value present & nonempty, identity key absent -> reject;
// B. value absent, identity key present -> reject;
// C. value present & nonempty, identity key present with empty string -> reject;
// D. value present as explicit nonnil empty slice, identity key absent -> accept;
// E. value has wrong dynamic type -> reject.
// For every rejecting case: initializes dest with an unrelated plane, asserts rollback,
// AttributedError attribution, ErrInvalidContribution, and zero live ID() calls.
// Also tests valid map-backed snapshot replay with zero live ID() calls.
func TestOrderedIdentityPlanes_MapBacked_MalformedStateAndRollback(t *testing.T) {
	t.Parallel()
	...
	assertRollbackAndAttribution := func(t *testing.T, dst *feature.ContributionSet, badFrozen feature.FrozenPlaneSet, expectedPlaneID string, calls *int) {
		t.Helper()
		snapBefore := dst.Freeze()

		require.Error(t, badFrozen.Validate(), "malformed map-backed snapshot must fail Validate()")

		err := badFrozen.ReplayTo(dst, "map-replay")
		require.Error(t, err, "malformed map-backed snapshot must fail ReplayTo()")

		var attrErr *feature.AttributedError
		require.True(t, errors.As(err, &attrErr), "error must be *AttributedError, got %T: %v", err, err)
		assert.Equal(t, "map-replay", attrErr.PluginID)
		assert.Equal(t, expectedPlaneID, attrErr.PlaneID)
		assert.True(t, errors.Is(err, feature.ErrInvalidContribution), "error must satisfy errors.Is(err, ErrInvalidContribution)")

		// Destination must remain unchanged (rollback)
		snapAfter := dst.Freeze()
		assert.Equal(t, feature.Get(snapBefore, feature.PlaneSubmitHooks), feature.Get(snapAfter, feature.PlaneSubmitHooks))
		if calls != nil {
			assert.Equal(t, 0, *calls, "no live ID() calls allowed during replay rejection")
		}
	}
	...
	t.Run("PlaneAttemptTransforms", func(t *testing.T) {
		...
		// Valid replay test
		callsValid := 0
		trValid := []request.AttemptTransform{callCountingAttemptTransform{id: "at-map-valid", calls: &callsValid}}
		mapValid := feature.NewFrozenPlaneSetFromMapForTest(
			map[string]any{feature.PlaneAttemptTransforms.ID: trValid},
			map[string]string{feature.PlaneAttemptTransforms.ID: "at-map-valid"},
		)
		require.NoError(t, mapValid.Validate())
		assert.Equal(t, 0, callsValid)
		dstValid := feature.NewContributionSet()
		require.NoError(t, mapValid.ReplayTo(dstValid, "map-plugin"))
		assert.Equal(t, 0, callsValid)
		dstFrozen := dstValid.Freeze()
		id, ok := feature.FrozenIdentity(dstFrozen, feature.PlaneAttemptTransforms)
		assert.True(t, ok)
		assert.Equal(t, "at-map-valid", id)

		// Table-driven malformed cases
		t.Run("CaseA_ValuePresentNonEmpty_IdentityKeyAbsent", func(t *testing.T) {
			t.Parallel()
			calls := 0
			tr := []request.AttemptTransform{callCountingAttemptTransform{id: "at-map-a", calls: &calls}}
			bad := feature.NewFrozenPlaneSetFromMapForTest(
				map[string]any{feature.PlaneAttemptTransforms.ID: tr},
				map[string]string{},
			)
			dst := initDest(t)
			assertRollbackAndAttribution(t, dst, bad, feature.PlaneAttemptTransforms.ID, &calls)
		})

		t.Run("CaseB_ValueAbsent_IdentityKeyPresent", func(t *testing.T) {
			t.Parallel()
			bad := feature.NewFrozenPlaneSetFromMapForTest(
				map[string]any{},
				map[string]string{feature.PlaneAttemptTransforms.ID: "at-map-b"},
			)
			dst := initDest(t)
			assertRollbackAndAttribution(t, dst, bad, feature.PlaneAttemptTransforms.ID, nil)
		})

		t.Run("CaseC_ValuePresentNonEmpty_IdentityKeyEmptyString", func(t *testing.T) {
			t.Parallel()
			calls := 0
			tr := []request.AttemptTransform{callCountingAttemptTransform{id: "at-map-c", calls: &calls}}
			bad := feature.NewFrozenPlaneSetFromMapForTest(
				map[string]any{feature.PlaneAttemptTransforms.ID: tr},
				map[string]string{feature.PlaneAttemptTransforms.ID: ""},
			)
			dst := initDest(t)
			assertRollbackAndAttribution(t, dst, bad, feature.PlaneAttemptTransforms.ID, &calls)
		})

		t.Run("CaseD_ValueExplicitEmptySlice_IdentityKeyAbsent_Accepted", func(t *testing.T) {
			t.Parallel()
			good := feature.NewFrozenPlaneSetFromMapForTest(
				map[string]any{feature.PlaneAttemptTransforms.ID: []request.AttemptTransform{}},
				map[string]string{},
			)
			require.NoError(t, good.Validate())
			dst := initDest(t)
			require.NoError(t, good.ReplayTo(dst, "map-replay"))
		})

		t.Run("CaseE_ValueWrongDynamicType", func(t *testing.T) {
			t.Parallel()
			bad := feature.NewFrozenPlaneSetFromMapForTest(
				map[string]any{feature.PlaneAttemptTransforms.ID: "wrong-dynamic-type"},
				map[string]string{feature.PlaneAttemptTransforms.ID: "at-map-e"},
			)
			dst := initDest(t)
			assertRollbackAndAttribution(t, dst, bad, feature.PlaneAttemptTransforms.ID, nil)
		})
	...
	t.Run("PlaneStreamObserverFactories", func(t *testing.T) {
		...
		// Valid replay test
		callsValid := 0
		sofValid := []response.StreamObserverFactory{callCountingStreamObserverFactory{id: "sof-map-valid", calls: &callsValid}}
		mapValid := feature.NewFrozenPlaneSetFromMapForTest(
			map[string]any{feature.PlaneStreamObserverFactories.ID: sofValid},
			map[string]string{feature.PlaneStreamObserverFactories.ID: "sof-map-valid"},
		)
		require.NoError(t, mapValid.Validate())
		assert.Equal(t, 0, callsValid)
		dstValid := feature.NewContributionSet()
		...
		assert.Equal(t, 0, callsValid)
		dstFrozen := dstValid.Freeze()
		id, ok := feature.FrozenIdentity(dstFrozen, feature.PlaneStreamObserverFactories)
		assert.True(t, ok)
		assert.Equal(t, "sof-map-valid", id)

		// Table-driven malformed cases
		t.Run("CaseA_ValuePresentNonEmpty_IdentityKeyAbsent", func(t *testing.T) {
			t.Parallel()
			calls := 0
			sof := []response.StreamObserverFactory{callCountingStreamObserverFactory{id: "sof-map-a", calls: &calls}}
			bad := feature.NewFrozenPlaneSetFromMapForTest(
				map[string]any{feature.PlaneStreamObserverFactories.ID: sof},
				map[string]string{},
			)
			dst := initDest(t)
			assertRollbackAndAttribution(t, dst, bad, feature.PlaneStreamObserverFactories.ID, &calls)
		})
		...
		t.Run("CaseB_ValueAbsent_IdentityKeyPresent", func(t *testing.T) {
			...
			bad := feature.NewFrozenPlaneSetFromMapForTest(
				map[string]any{},
				map[string]string{feature.PlaneStreamObserverFactories.ID: "sof-map-b"},
			)
			dst := initDest(t)
			assertRollbackAndAttribution(t, dst, bad, feature.PlaneStreamObserverFactories.ID, nil)
		})

		t.Run("CaseC_ValuePresentNonEmpty_IdentityKeyEmptyString", func(t *testing.T) {
			t.Parallel()
			calls := 0
			sof := []response.StreamObserverFactory{callCountingStreamObserverFactory{id: "sof-map-c", calls: &calls}}
			bad := feature.NewFrozenPlaneSetFromMapForTest(
				map[string]any{feature.PlaneStreamObserverFactories.ID: sof},
				map[string]string{feature.PlaneStreamObserverFactories.ID: ""},
			)
			dst := initDest(t)
			assertRollbackAndAttribution(t, dst, bad, feature.PlaneStreamObserverFactories.ID, &calls)
		})
		...
		t.Run("CaseD_ValueExplicitEmptySlice_IdentityKeyAbsent_Accepted", func(t *testing.T) {
			t.Parallel()
			good := feature.NewFrozenPlaneSetFromMapForTest(
				map[string]any{feature.PlaneStreamObserverFactories.ID: []response.StreamObserverFactory{}},
				map[string]string{},
			)
			require.NoError(t, good.Validate())
			dst := initDest(t)
		...
		t.Run("CaseE_ValueWrongDynamicType", func(t *testing.T) {
			t.Parallel()
			bad := feature.NewFrozenPlaneSetFromMapForTest(
				map[string]any{feature.PlaneStreamObserverFactories.ID: 12345},
				map[string]string{feature.PlaneStreamObserverFactories.ID: "sof-map-e"},
			)
			dst := initDest(t)
			assertRollbackAndAttribution(t, dst, bad, feature.PlaneStreamObserverFactories.ID, nil)
		})
	...
	t.Run("PlaneCompactionPreservers", func(t *testing.T) {
		...
		// Valid replay test
		callsValid := 0
		cpValid := []compaction.Preserver{callCountingCompactionPreserver{stubPreserver: stubPreserver{id: "cp-map-valid"}, calls: &callsValid}}
		mapValid := feature.NewFrozenPlaneSetFromMapForTest(
			map[string]any{feature.PlaneCompactionPreservers.ID: cpValid},
			map[string]string{feature.PlaneCompactionPreservers.ID: "cp-map-valid"},
		)
		require.NoError(t, mapValid.Validate())
		assert.Equal(t, 0, callsValid)
		dstValid := feature.NewContributionSet()
		...
		assert.True(t, ok)
		assert.Equal(t, "cp-map-valid", id)

		// Table-driven malformed cases
		t.Run("CaseA_ValuePresentNonEmpty_IdentityKeyAbsent", func(t *testing.T) {
			t.Parallel()
			calls := 0
			cp := []compaction.Preserver{callCountingCompactionPreserver{stubPreserver: stubPreserver{id: "cp-map-a"}, calls: &calls}}
			bad := feature.NewFrozenPlaneSetFromMapForTest(
				map[string]any{feature.PlaneCompactionPreservers.ID: cp},
				map[string]string{},
			)
			dst := initDest(t)
			assertRollbackAndAttribution(t, dst, bad, feature.PlaneCompactionPreservers.ID, &calls)
		})
		...
		t.Run("CaseB_ValueAbsent_IdentityKeyPresent", func(t *testing.T) {
			...
			bad := feature.NewFrozenPlaneSetFromMapForTest(
				map[string]any{},
				map[string]string{feature.PlaneCompactionPreservers.ID: "cp-map-b"},
			)
			dst := initDest(t)
			assertRollbackAndAttribution(t, dst, bad, feature.PlaneCompactionPreservers.ID, nil)
		})

		t.Run("CaseC_ValuePresentNonEmpty_IdentityKeyEmptyString", func(t *testing.T) {
			t.Parallel()
			calls := 0
			cp := []compaction.Preserver{callCountingCompactionPreserver{stubPreserver: stubPreserver{id: "cp-map-c"}, calls: &calls}}
			bad := feature.NewFrozenPlaneSetFromMapForTest(
				map[string]any{feature.PlaneCompactionPreservers.ID: cp},
				map[string]string{feature.PlaneCompactionPreservers.ID: ""},
			)
			dst := initDest(t)
			assertRollbackAndAttribution(t, dst, bad, feature.PlaneCompactionPreservers.ID, &calls)
		})
		...
		t.Run("CaseD_ValueExplicitEmptySlice_IdentityKeyAbsent_Accepted", func(t *testing.T) {
			t.Parallel()
			good := feature.NewFrozenPlaneSetFromMapForTest(
				map[string]any{feature.PlaneCompactionPreservers.ID: []compaction.Preserver{}},
				map[string]string{},
			)
			require.NoError(t, good.Validate())
			dst := initDest(t)
		...
		t.Run("CaseE_ValueWrongDynamicType", func(t *testing.T) {
			t.Parallel()
			bad := feature.NewFrozenPlaneSetFromMapForTest(
				map[string]any{feature.PlaneCompactionPreservers.ID: false},
				map[string]string{feature.PlaneCompactionPreservers.ID: "cp-map-e"},
			)
			dst := initDest(t)
			assertRollbackAndAttribution(t, dst, bad, feature.PlaneCompactionPreservers.ID, nil)
		})
...
func TestOrderedIdentityPlanes_GenerationBinderReplayRejected(t *testing.T) {
	...
	t.Run("MapBacked_PlaneAttemptTransforms", func(t *testing.T) {
		t.Parallel()
		calls := 0
		tr := []request.AttemptTransform{callCountingAttemptTransform{id: "at-map-1", calls: &calls}}
		...
		err := mapFrozen.ReplaySourceTo(dst, feature.SourceGenerationBinder, "map-binder")
		require.Error(t, err)
		assert.True(t, errors.Is(err, feature.ErrUnsupportedReplaySource))

		var attrErr *feature.AttributedError
		require.True(t, errors.As(err, &attrErr))
		assert.Equal(t, "map-binder", attrErr.PluginID)
		assert.Equal(t, feature.PlaneAttemptTransforms.ID, attrErr.PlaneID)

		assert.Equal(t, 0, calls, "rejection must happen before any live ID calls")

		snapAfter := dst.Freeze()
		assert.Equal(t, feature.Get(snapBefore, feature.PlaneSubmitHooks), feature.Get(snapAfter, feature.PlaneSubmitHooks))
	})

	t.Run("MapBacked_PlaneStreamObserverFactories", func(t *testing.T) {
		t.Parallel()
		calls := 0
		sof := []response.StreamObserverFactory{callCountingStreamObserverFactory{id: "sof-map-1", calls: &calls}}
		mapFrozen := feature.NewFrozenPlaneSetFromMapForTest(
			map[string]any{feature.PlaneStreamObserverFactories.ID: sof},
	...
	t.Run("MapBacked_PlaneCompactionPreservers", func(t *testing.T) {
		t.Parallel()
		calls := 0
		cp := []compaction.Preserver{callCountingCompactionPreserver{stubPreserver: stubPreserver{id: "cp-map-1"}, calls: &calls}}
		mapFrozen := feature.NewFrozenPlaneSetFromMapForTest(
			map[string]any{feature.PlaneCompactionPreservers.ID: cp},

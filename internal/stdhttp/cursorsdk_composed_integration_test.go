...
models:
  source: inline
  items:
    - canonical_id: cursor/gpt-5.3-codex
...
`, acpExe, ws)
	...
	cfg := &config.Config{
		...
		Plugins: config.PluginsConfig{
			Backends: []config.PluginConfig{
				...
				{Kind: cursorcliacp.ID, ID: "cursor-acp", Enabled: true, Config: acpNode},
			},
		},
	}
	...
	if byKind[cursorsdk.ID] != "cursor-sdk" || byKind[cursorcliacp.ID] != "cursor-acp" {

...
var benchmarkOutcome toolcallrepair.Outcome
...
func BenchmarkEngineRepair(b *testing.B) {
	...
	if _, err := engine.Repair(toolcallrepair.Input{ToolName: "get_weather", ArgsJSON: []byte(`{"location":"NYC"}`), Catalog: catalog}); err != nil {
		b.Fatal(err)
	}
	cases := []struct {
		name string
	...
	for _, tc := range cases {
		b.Run(tc.name, func(b *testing.B) {
			b.ReportAllocs()
			for b.Loop() {
				out, err := engine.Repair(toolcallrepair.Input{
...
func TestEngineValidHotPathAllocationBudget(t *testing.T) {
	...
	catalog := []lipapi.ToolDef{{Name: "get_weather", Parameters: schema}}
	engine := toolcallrepair.NewEngine()
	in := toolcallrepair.Input{ToolName: "get_weather", ArgsJSON: []byte(`{"location":"NYC"}`), Catalog: catalog}
	if _, err := engine.Repair(in); err != nil {
		t.Fatal(err)
	}
	allocs := testing.AllocsPerRun(100, func() {
		out, err := engine.Repair(in)
		if err != nil || out.Kind != toolcallrepair.OutcomePass {

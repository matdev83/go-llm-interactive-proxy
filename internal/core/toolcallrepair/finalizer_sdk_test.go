...
func policyFrom(onUnrepairable string, maxArgs int) toolcallrepair.FinalizerPolicy {
	p := toolcallrepair.FinalizerPolicy{
		ID:             toolcallrepair.DefaultFinalizerID,
		MaxArgsBytes:   toolcallrepair.DefaultMaxArgsBytes,
		OnUnrepairable: toolcallrepair.OnUnrepairablePassThrough,
...
func TestFinalizer_CoreBehaviors(t *testing.T) {
	...
	cases := []struct {
		name           string
		policy         toolcallrepair.FinalizerPolicy
		toolName       string
		args           []byte
		...
		},
		{
			...
			policy:     policyFrom(toolcallrepair.OnUnrepairablePassThrough, 0),
			toolName:   "get_weather",
			args:       []byte(`{}`),
			catalog:    weatherCatalog,
			wantAction: toolcall.ActionPass,
		...
		},
		{
			...
			policy:       policyFrom(toolcallrepair.OnUnrepairableError, 0),
			toolName:     "get_weather",
			args:         []byte(`{}`),
			catalog:      weatherCatalog,
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			fin := toolcallrepair.NewFinalizer(tc.policy)
...
func TestFinalizer_ResolvedToolExactFastPath(t *testing.T) {
	...
	fin := toolcallrepair.NewFinalizer(policyFrom("", 0))

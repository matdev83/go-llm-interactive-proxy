...
func TestEngineContract_Fixtures(t *testing.T) {
	...
	for _, name := range idx.Cases {
		t.Run(name, func(t *testing.T) {
			...
			if fx.Normalize != nil {
				got := toolcallrepair.NormalizeASCIIName(fx.Normalize.Input)
			...
			if fx.Generate != nil {
				...
				if len(args) <= toolcallrepair.DefaultMaxArgsBytes {
			...
			res, err := eng.Repair(toolcallrepair.Input{
				ToolCallID:   "call-1",
				ToolName:     fx.Input.ToolName,
				ArgsJSON:     args,
				Catalog:      catalog,
				MaxArgsBytes: toolcallrepair.DefaultMaxArgsBytes,
			})
			if err != nil {
			...
			case "pass":
				if res.Kind != toolcallrepair.OutcomePass {
					t.Fatalf("kind=%v want pass", res.Kind)
				}
			case "rewrite":
				if res.Kind != toolcallrepair.OutcomeRewrite {
					t.Fatalf("kind=%v want rewrite", res.Kind)
				}
			case "unrepairable":
				if res.Kind != toolcallrepair.OutcomeUnrepairable {
			...
			if fx.Expect.MustNotMutate && res.Kind == toolcallrepair.OutcomeRewrite {
			...
			if fx.Expect.RequireSchemaValid {
				...
				if err := toolcallrepair.ValidateArgsAgainstSchema(target, schema); err != nil {
...
func TestNormalizeASCIIName_Contract(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			got := toolcallrepair.NormalizeASCIIName(tc.in)
			if got != tc.want {
...
func TestValidateArgsAgainstSchema_ValidAndInvalid(t *testing.T) {
	...
	if err := toolcallrepair.ValidateArgsAgainstSchema([]byte(`{"location":"NYC"}`), schema); err != nil {
		t.Fatalf("valid args: %v", err)
	}
	err := toolcallrepair.ValidateArgsAgainstSchema([]byte(`{"location":1}`), schema)
	if err == nil {
	...
	if se.Kind != toolcallrepair.SchemaKindValidationFailed {
		t.Fatalf("kind=%q want %q", se.Kind, toolcallrepair.SchemaKindValidationFailed)
	}
}

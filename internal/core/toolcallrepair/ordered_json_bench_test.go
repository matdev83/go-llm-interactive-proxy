...
func benchParseAlone(b *testing.B, name string, payload []byte, wantErr bool) {
	...
	b.Run(name, func(b *testing.B) {
		...
		b.ResetTimer()
		for b.Loop() {
			v, err := toolcallrepair.ExportParseOrderedJSON(payload)
			if wantErr {
				if err == nil {
...
func benchPreflightPlusParse(b *testing.B, name string, payload []byte, wantPreflightErr, wantParseErr bool) {
	...
	b.Run(name, func(b *testing.B) {
		...
		b.ResetTimer()
		for b.Loop() {
			err := toolcallrepair.ExportPreflightArgsJSON(context.Background(), payload, toolcallrepair.DefaultMaxArgsBytes)
			if wantPreflightErr {
				if err == nil {
...
func BenchmarkEngineRepair_parserContribution(b *testing.B) {
	...
	in := toolcallrepair.Input{ToolName: "bulk", ArgsJSON: payload, Catalog: catalog}

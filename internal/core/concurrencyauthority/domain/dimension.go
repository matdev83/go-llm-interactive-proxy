...
// Dimensions are the safe attribution fields used for lease matching and identity.
...
// DimensionKey is a deterministic encoding of Dimensions for capacity grouping.
...
func (d Dimensions) KeyErr() (DimensionKey, error) {
	...
	for key := range d.PolicyLabels {
		if !isSafeLabelKey(key) {
			return "", fmt.Errorf("concurrencyauthority: invalid policy label key %q", key)
		}
...
// DimensionMatcher matches one attribution field. An unknown matcher value is
// a wildcard; MatchUnknown requires an unknown actual.
...
// DimensionsFromScope projects a principal scope view onto lease dimensions.

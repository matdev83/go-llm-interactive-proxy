...
// StableLeaseID returns lease_id = stable(namespace, rule_version, logical_request_id, principal/scope).
func StableLeaseID(namespace, ruleVersion, logicalRequestID string, dims Dimensions) string {
	sum := sha256.Sum256([]byte(fmt.Sprintf("%s|%s|%s|%s", namespace, ruleVersion, logicalRequestID, dims.Key())))

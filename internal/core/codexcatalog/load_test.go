...
func TestLoad_DisabledUsesShippedFallback(t *testing.T) {
	t.Parallel()
	c, src, err := codexcatalog.Load(context.Background(), codexcatalog.LoadOptions{Enabled: false})
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
	if src != codexcatalog.SourceShippedFallback {
		t.Fatalf("source = %q, want %q", src, codexcatalog.SourceShippedFallback)
	}
...
func TestLoad_DiscoveryFailureFallsBackToShipped(t *testing.T) {
	...
	cancel()
	c, src, err := codexcatalog.Load(ctx, codexcatalog.LoadOptions{Enabled: true, Timeout: time.Second})
	if err != nil {
		t.Fatalf("Load: %v", err)
	}
	if src != codexcatalog.SourceShippedFallback {
...
func TestLoad_DiscoveryFailureFallsBackToOverride(t *testing.T) {
	...
	ctx, cancel := context.WithCancel(context.Background())
	cancel()
	c, src, err := codexcatalog.Load(ctx, codexcatalog.LoadOptions{
		Enabled:      true,
		FallbackPath: path,
		Timeout:      time.Second,
	})
	...
	if src != codexcatalog.SourceOverrideFallback {
		t.Fatalf("source = %q, want %q", src, codexcatalog.SourceOverrideFallback)
	}

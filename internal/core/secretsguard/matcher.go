...
func (m *Matcher) ScanBytes(input []byte) []secretguard.Finding {
...
func (m *Matcher) ScanString(input string) []secretguard.Finding {
	if input == "" {
		return nil
	}
...
func (m *Matcher) RedactBytes(input []byte) (redacted []byte, findings []secretguard.Finding) {
	if len(input) == 0 {
		return nil, nil
	}
...
func (m *Matcher) RedactString(input string) (redacted string, findings []secretguard.Finding) {
	if input == "" {
		return "", nil
	}
...
func (m *Matcher) findingsFromCounts(counts map[int]int) []secretguard.Finding {
	if len(counts) == 0 {
		return nil
	}
	...
	out := make([]secretguard.Finding, 0, len(idxs))
	for _, i := range idxs {
		e := m.entries[i]
		out = append(out, secretguard.Finding{

...
type CatalogInput struct {
	...
	SourceCategory    secretguard.SourceCategory
}
...
func BuildCatalog(raw []CatalogInput, minSecretBytes int) (*Catalog, error) {
	...
	type accum struct {
		...
		nameCategory      map[string]secretguard.SourceCategory
	}
	...
	for i := range raw {
		...
		if a == nil {
			...
			a = &accum{
				...
				nameCategory:      make(map[string]secretguard.SourceCategory, 1+len(in.Aliases)),
			}
	...
	for _, group := range byHash {
		for _, a := range group {
			...
			if cat == "" {
				cat = secretguard.SourceCategoryUnknown
			}
...
func (c *Catalog) SourceCategories() []string {
	...
	for _, e := range c.entries {
		...
		if cat == "" {
			cat = string(secretguard.SourceCategoryUnknown)
		}
		if _, ok := seen[cat]; ok {
			continue
		}

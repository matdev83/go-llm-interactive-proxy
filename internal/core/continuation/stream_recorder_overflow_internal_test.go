...
type coreOverflowBoundaryRecorder struct{ calls int }

func (r *coreOverflowBoundaryRecorder) RecordTerminal(context.Context, lipcont.ContinuationRecord) error {
	r.calls++
	return nil
}
...
func TestStreamRecorderEventBytesInt64Boundary(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			t.Parallel()
			backend := &coreOverflowBoundaryRecorder{}
			cleanupCalls := 0
			r := NewStreamRecorder(backend, lipcont.ContinuationRecord{}, func() { cleanupCalls++ })
...
func TestStreamRecorderMaxRecordBytesStillOverflows(t *testing.T) {
	...
	backend := &coreOverflowBoundaryRecorder{}
	cleanupCalls := 0
	r := NewStreamRecorder(backend, lipcont.ContinuationRecord{
		Policy: lipcont.StoragePolicy{Limits: lipcont.StorageLimits{MaxRecordBytes: 4}},
	}, func() { cleanupCalls++ })

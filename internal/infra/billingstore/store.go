...
// RequiredMigrationNames are the billing migration history entries required
// after a successful Migrate. Existing installations may retain older rows.
var RequiredMigrationNames = []string{BaselineMigrationName, AuthorizationSchemaMigrationName, Phase4MigrationName, Phase6MigrationName, Phase7MigrationName, SessionIDMigrationName}

// Config controls ownership and stable store identity for a billing database.
type Config struct {
	StoreID string
}

// DurableStore owns billing persistence mechanics. Core billing ports do not
// expose this type; application code should depend on narrow domain contracts.
type DurableStore struct {
	db        *bun.DB
	storeID   string
	nonOwning bool

	// settlementFaultHook is test-only fault injection kept inside the store
	// package so production callers cannot manufacture partial financial state.
...
func VerifySchema(ctx context.Context, database *bun.DB) error {
	...
	for _, table := range []string{
		"billing_accounts", "billing_account_openings", "billing_reconciliation_events", "billing_account_policy_events", "authorization_holds",
		...
		// Empty probe: table exists when the statement runs; zero rows yields ErrNoRows.
		var probe int
	...
	if database.Dialect().Name() == dialect.SQLite {
		...
		for _, index := range []string{"idx_billing_holds_account_status", "idx_billing_processing_status", "idx_billing_journal_account_sequence", "idx_billing_journal_source", journalReversalUniqueIndex, sessionAccountIndex} {
			var name string
		...
		tableFragments := map[string][]string{
			...
			"billing_account_policy_events": {"FOREIGN KEY(account_id) REFERENCES billing_accounts", "UNIQUE(account_id, source_key)"}, "authorization_holds": {"CHECK", "authorization_id", "pricing_ref", "charge_policy_ref", "balance_before_nano", "spendable_after_nano", "FOREIGN KEY(account_id) REFERENCES billing_accounts", "UNIQUE(account_id, tur_key)"},
	...
	checks := []struct {
		...
		{"authorization identity column", `SELECT column_name FROM information_schema.columns WHERE table_schema = current_schema() AND table_name = 'authorization_holds' AND column_name = 'authorization_id' LIMIT 1`, nil, []string{"authorization_id"}},
		{"authorization snapshot column", `SELECT column_name FROM information_schema.columns WHERE table_schema = current_schema() AND table_name = 'authorization_holds' AND column_name = 'balance_before_nano' LIMIT 1`, nil, []string{"balance_before_nano"}},
		...
		{"authorization schema migration history", `SELECT name FROM bun_billing_migrations WHERE name = ? LIMIT 1`, []any{AuthorizationSchemaMigrationName}, []string{AuthorizationSchemaMigrationName}},
		...
		{"hold status index", `SELECT indexdef FROM pg_indexes WHERE schemaname = current_schema() AND indexname = ? LIMIT 1`, []any{"idx_billing_holds_account_status"}, []string{"account_id", "status"}},
		...
		{"hold account foreign key", `SELECT pg_get_constraintdef(c.oid) FROM pg_constraint c JOIN pg_class t ON t.oid = c.conrelid JOIN pg_namespace n ON n.oid = t.relnamespace WHERE n.nspname = current_schema() AND t.relname = 'authorization_holds' AND c.contype = 'f' LIMIT 1`, nil, []string{"FOREIGN KEY", "billing_accounts"}},
...
func NewDurableStore(ctx context.Context, database *bun.DB, cfg Config) (*DurableStore, error) {
	...
	return openStore(ctx, database, cfg, false)
}

func OpenStore(ctx context.Context, database *bun.DB, cfg Config) (*DurableStore, error) {
	return openStore(ctx, database, cfg, true)
}

func openStore(ctx context.Context, database *bun.DB, cfg Config, nonOwning bool) (*DurableStore, error) {
	if ctx == nil {
	...
	return &DurableStore{db: database, storeID: strings.TrimSpace(cfg.StoreID), nonOwning: nonOwning}, nil
}

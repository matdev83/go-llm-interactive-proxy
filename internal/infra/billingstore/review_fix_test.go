...
func TestIsUniqueViolationClassifiesDialectCodesNotConstraintText(t *testing.T) {
	...
	_, uniqueErr := store.db.NewRaw(`INSERT INTO billing_accounts(account_id, currency, mode, credit_limit_nano, balance_nano, opening_balance_nano, version, state, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)`,
		"unique-classify", "USD", string(billing.AccountPrepaid), int64(0), int64(10), int64(10), uint64(1), string(billing.AccountReady), "now", "now").Exec(ctx)
	if uniqueErr == nil {
	...
	_, checkErr := store.db.NewRaw(`UPDATE billing_accounts SET credit_limit_nano = -1 WHERE account_id = ?`, "unique-classify").Exec(ctx)
	if checkErr == nil {
...
func TestSQLiteReadIntegrityReportMissingOpeningReturnsIssue(t *testing.T) {
	...
	if _, err := store.db.NewRaw(`INSERT INTO billing_accounts(account_id, currency, mode, credit_limit_nano, balance_nano, opening_balance_nano, version, state, created_at, updated_at) VALUES (?,?,?,?,?,?,?,?,?,?)`,
		"report-missing-opening", "USD", string(billing.AccountPrepaid), int64(0), int64(50), int64(50), uint64(1), string(billing.AccountReady), now, now).Exec(ctx); err != nil {
		t.Fatal(err)
	}

...
// --- Acceptance Criteria 3: All 16 Handled Slice Planes Append Order ---

// TestOverlayExtensions_AllSlicePlanesAppendOrder pins requirement 1.1, 5.1:
// - For all 16 slice planes handled by overlayExtensions, source elements append after destination elements in registration order.
func TestOverlayExtensions_AllSlicePlanesAppendOrder(t *testing.T) {
	...
	require.Len(t, dst.UsageObservers, 2)
	require.Len(t, dst.RawCaptureSinks, 2)
	require.Equal(t, []string{"d-redact", "s-redact"}, []string{dst.TrafficRedactors[0].ID(), dst.TrafficRedactors[1].ID()})

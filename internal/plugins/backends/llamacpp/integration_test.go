...
func TestNew_transportCaps_chatCompletionsOnly(t *testing.T) {
	...
	be := llamacpp.New(llamacpp.Config{BaseURL: "http://127.0.0.1:1"})
	...
	resolved := be.ResolveTransportCaps(context.Background(), testCall(), testCandidate("llamacpp:local-model"))
...
func TestNew_openRejectsResponsesAPI(t *testing.T) {
	...
	_, err := be.Open(context.Background(), responsesTestCall(), testCandidate("llamacpp:local-model"))
	if err == nil {
		t.Fatal("expected error for responses API")
	}
	if !strings.Contains(err.Error(), "llamacpp: responses API is not available") {
...
func TestNew_openRejectsResponsesAPI_viaOpenAIResponsesExtension(t *testing.T) {
	t.Parallel()
	var mu sync.Mutex
	var paths []string
	...
	call := testCall()
	call.Extensions = map[string]json.RawMessage{
		"openairesponses.model": json.RawMessage(`"local-model"`),
	}
	call.Invocation = lipapi.Invocation{
		Operation:     lipapi.OperationOpenAIResponses,
		DeliveryMode:  lipapi.DeliveryModeNonStreaming,
	...
	_, err := be.Open(context.Background(), call, testCandidate("llamacpp:local-model"))
	if err == nil {
		t.Fatal("expected error for responses API via openairesponses.model")
	}
	if !strings.Contains(err.Error(), "llamacpp: responses API is not available") {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestNew_buildsBackendIDAndChatCompletions(t *testing.T) {
	...
	if len(be.BackendPrefixes) != 1 || be.BackendPrefixes[0] != llamacpp.ID {
		t.Fatalf("BackendPrefixes = %#v", be.BackendPrefixes)
	}

	es, err := be.Open(context.Background(), testCall(), testCandidate("llamacpp:local-model"))
	if err != nil {
		t.Fatal(err)
	}
	...
	for _, ev := range events {
		if ev.Kind == lipapi.EventTextDelta && strings.Contains(ev.Delta, "llamacpp-ok") {
			hasText = true
		}
	}
	if !hasText {
...
func TestNew_inventoryMapsCatalogAndFallsBack(t *testing.T) {
	...
	be := llamacpp.New(llamacpp.Config{
		...
		Discovery: llamacpp.DiscoveryConfig{
	...
	want := map[string]string{
		...
		"unknown-local": "llamacpp/unknown-local",
	}

...
func TestGPT55Downgrade_reactive400CustomSourceRetriesWithTarget(t *testing.T) {
	t.Parallel()
	var calls atomic.Int32
	var lastModel atomic.Value
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		lastModel.Store(model)
		if model == "custom-src" {
			http.Error(w, `{"error":{"message":"custom-src is not available on free plan"}}`, http.StatusBadRequest)
			return
		}
		refbackend.New(refbackend.Config{Token: "tok"}).Handler().ServeHTTP(w, r)
	}))
	...
	es, err := be.Open(context.Background(), sampleCall(), routing.AttemptCandidate{
		Primary: routing.Primary{Model: "custom-src"},
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	if got != "custom-dst" {
		t.Fatalf("final model = %q, want custom-dst", got)
	}
}

func TestGPT55Downgrade_reactiveWSFreePlanRetriesWithTarget(t *testing.T) {
	t.Parallel()
	srv := refbackend.New(refbackend.Config{
		Token:               "tok",
		OutputText:          "ok",
		ForcedWSRejectModel: "gpt-5.5",
	})
	ts := httptest.NewServer(srv.Handler())
	t.Cleanup(ts.Close)

...
func baseInput(t *testing.T) cursorsdk.Input {
	t.Helper()
	return cursorsdk.Input{
...
func TestNormalize_defaults(t *testing.T) {
	...
	cfg, err := cursorsdk.Normalize(baseInput(t), "")
	require.NoError(t, err)
	...
	require.Equal(t, cursorsdk.SandboxRequired, cfg.SandboxMode)
	require.False(t, cfg.AutoReview)
	require.Equal(t, cursorsdk.MaxFrameBytes, 16<<20)
	require.Equal(t, cursorsdk.MaxPromptBytes, 8<<20)
	require.Equal(t, cursorsdk.MaxMCPConfigBytes, 256<<10)
	require.Equal(t, cursorsdk.MaxStderrRetainBytes, 8<<10)
...
func TestNormalize_apiKeyFallbackAndPrecedence(t *testing.T) {
	...
	in := baseInput(t)
	in.APIKey = ""
	cfg, err := cursorsdk.Normalize(in, "env-cursor-key")
	require.NoError(t, err)
...
func TestNormalize_sdkSettingSourcesPinned(t *testing.T) {
	...
	cfg, err := cursorsdk.Normalize(in, "")
	require.NoError(t, err)
...
func TestNormalize_bounds(t *testing.T) {
	...
	cases := []struct {
		name string
		mut  func(*cursorsdk.Input)
	}{
		{"max_agents_low", func(in *cursorsdk.Input) { v := 0; in.MaxAgents = &v }},
		{"max_agents_high", func(in *cursorsdk.Input) { v := 33; in.MaxAgents = &v }},
		{"max_runs_high", func(in *cursorsdk.Input) { v := 9; in.MaxConcurrentRuns = &v }},
		{"runs_gt_agents", func(in *cursorsdk.Input) {
			a, r := 4, 5
			in.MaxAgents = &a
			in.MaxConcurrentRuns = &r
		}},
		{"start_low", func(in *cursorsdk.Input) { v := 0.5; in.BridgeStartTimeoutSeconds = &v }},
		{"start_high", func(in *cursorsdk.Input) { v := 121.0; in.BridgeStartTimeoutSeconds = &v }},
		{"cancel_low", func(in *cursorsdk.Input) { v := 0.05; in.CancelTimeoutSeconds = &v }},
		{"cancel_high", func(in *cursorsdk.Input) { v := 31.0; in.CancelTimeoutSeconds = &v }},
		{"shutdown_low", func(in *cursorsdk.Input) { v := 0.5; in.ShutdownTimeoutSeconds = &v }},
		{"shutdown_high", func(in *cursorsdk.Input) { v := 121.0; in.ShutdownTimeoutSeconds = &v }},
		{"idle_low", func(in *cursorsdk.Input) { v := 0.5; in.AgentIdleTimeoutSeconds = &v }},
		{"idle_high", func(in *cursorsdk.Input) {
...
func TestConfig_argvAndSelectHostEnvNeverCarryAPIKey(t *testing.T) {
	...
	selected := cursorsdk.SelectHostEnv(host, cfg.BridgeEnvAllowlist)
...
func TestScaffold_retainsNormalizedConfig(t *testing.T) {
	...
	require.Equal(t, cursorsdk.SandboxRequired, sc.SandboxMode())

...
func TestHandlerSpecConcurrentFirstUse(t *testing.T) {
	h := &Handler{DefaultRouteSelector: "stub:claude"}
	const goroutines = 64
	...
	first := h.spec()
	if first.Config.DefaultRouteSelector != "stub:claude" {
		t.Fatalf("spec route selector = %q, want stub:claude", first.Config.DefaultRouteSelector)
	}

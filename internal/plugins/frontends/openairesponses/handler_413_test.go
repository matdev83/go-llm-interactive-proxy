...
func TestHandler_JSONGuardDepthExactAcceptsAndRejectsPlusOne(t *testing.T) {
	...
	t.Run("exact_128", func(t *testing.T) {
		...
		exec := &recordingExecutor{}
		h := &openairesponses.Handler{Exec: exec, DefaultRouteSelector: "stub:gpt-4o-mini"}
		body := `{"model":"gpt-4o-mini","input":"hi","_nest":` + nest(127, "1") + `}`
		req := httptest.NewRequest(http.MethodPost, "/v1/responses", strings.NewReader(body))
		rr := httptest.NewRecorder()
		h.ServeHTTP(rr, req)
	...
	t.Run("depth_129", func(t *testing.T) {
		...
		exec := &recordingExecutor{}
		h := &openairesponses.Handler{Exec: exec, DefaultRouteSelector: "stub:gpt-4o-mini"}
		body := `{"model":"gpt-4o-mini","input":"hi","_nest":` + nest(128, `"`+marker+`"`) + `}`
		req := httptest.NewRequest(http.MethodPost, "/v1/responses", strings.NewReader(body))
		rr := httptest.NewRecorder()
		h.ServeHTTP(rr, req)
		if rr.Code != http.StatusBadRequest {
...
func TestHandler_JSONGuardRejectsInvalidUTF8(t *testing.T) {
	...
	exec := &recordingExecutor{}
	h := &openairesponses.Handler{Exec: exec, DefaultRouteSelector: "stub:gpt-4o-mini"}
	body := []byte(`{"model":"gpt-4o-mini","input":"`)
...
func TestHandler_JSONGuardAcceptsDuplicateMembers(t *testing.T) {
	...
	exec := &recordingExecutor{}
	h := &openairesponses.Handler{Exec: exec, DefaultRouteSelector: "stub:gpt-4o-mini"}
	body := `{"model":"bad","model":"gpt-4o-mini","input":"hi"}`
	req := httptest.NewRequest(http.MethodPost, "/v1/responses", strings.NewReader(body))
	rr := httptest.NewRecorder()
	h.ServeHTTP(rr, req)
	if rr.Code != http.StatusOK {
...
func TestHandler_invalidJSONDoesNotEchoBodyMarker(t *testing.T) {
	...
	exec := &recordingExecutor{}
	h := &openairesponses.Handler{Exec: exec, DefaultRouteSelector: "stub:gpt-4o-mini"}
	req := httptest.NewRequest(http.MethodPost, "/v1/responses", strings.NewReader(`{"model":"m","input":"`+marker+`"`))
	rr := httptest.NewRecorder()

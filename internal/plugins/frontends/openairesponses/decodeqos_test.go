...
func TestHandler_decodeLimiterSaturationReturns503WithoutExecutor(t *testing.T) {
	...
	h := &openairesponses.Handler{Exec: exec, DefaultRouteSelector: "stub:gpt-4o-mini", DecodeLimiter: limiter}
...
func TestHandler_decodeLimiterReleasesAfterDecodeFailureAndSuccess(t *testing.T) {
	...
	limiter := decodeqos.NewLimiter(1)
	h := &openairesponses.Handler{Exec: &recordingExecutor{}, DefaultRouteSelector: "stub:gpt-4o-mini", DecodeLimiter: limiter}

	badReq := httptest.NewRequest(http.MethodPost, "/v1/responses", strings.NewReader(`{"model":"m"`))

...
func TestHandlerSpecConcurrentFirstUse(t *testing.T) {
	h := &Handler{DefaultRouteSelector: "stub:gemini"}
	const goroutines = 64
	...
	first := h.spec()
	if first.Config.DefaultRouteSelector != "stub:gemini" {
		t.Fatalf("spec route selector = %q, want stub:gemini", first.Config.DefaultRouteSelector)
	}

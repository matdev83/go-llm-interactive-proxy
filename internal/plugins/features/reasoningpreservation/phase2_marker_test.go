...
func TestGenericFeatureDoesNotIssueCompanionTrustMarkers(t *testing.T) {
	...
	_, err := reasoningpreservation.NewAttemptTransform(cfg, store).HandleAttempt(context.Background(), &call, request.AttemptMeta{BackendID: "backend"}, request.Services{})
	if err != nil {
		t.Fatal(err)
	}

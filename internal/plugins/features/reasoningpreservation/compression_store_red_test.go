...
func TestCompression_Reserve_PendingPerSessionBound(t *testing.T) {
	...
	policy := "v1"
	_, err := cs.ReserveCompression(context.Background(), p, "t1", d1, policy)
	require.NoError(t, err)
	_, err = cs.ReserveCompression(context.Background(), p, "t2", d2, policy)
	require.Error(t, err)
...
func TestCompression_Reserve_PendingTotalBound(t *testing.T) {
	...
	for i, p := range parts {
		...
		_, _ = cs.ReserveCompression(context.Background(), p, fmt.Sprintf("id-%d", i), d, policy)
		// first two succeed
	}
	...
	_ = p3 // already
	_, err := cs.ReserveCompression(context.Background(), pFresh, "fresh", dFresh, policy)
	require.Error(t, err)
	var be *reasoningpreservation.BudgetError
	require.ErrorAs(t, err, &be)
...
func TestCompression_Attach_SurrogatePerTurnBound(t *testing.T) {
	...
	policy := "v1"
	res, err := cs.ReserveCompression(context.Background(), p, "t1", d, policy)
	require.NoError(t, err)
...
func TestCompression_Attach_SurrogatePerSessionBound(t *testing.T) {
	...
	d1 := appendArtifactForCompression(t, cs, p, "t1", 32)
	d2 := appendArtifactForCompression(t, cs, p, "t2", 32)
	res1, err := cs.ReserveCompression(context.Background(), p, "t1", d1, policy)
	require.NoError(t, err)
	...
	res2, err := cs.ReserveCompression(context.Background(), p, "t2", d2, policy)
	require.NoError(t, err)
...
func TestCompression_Attach_SurrogateTotalBound(t *testing.T) {
	...
	res1, err := cs.ReserveCompression(context.Background(), p1, "t1", d1, policy)
	require.NoError(t, err)
	...
	res2, err := cs.ReserveCompression(context.Background(), p2, "t2", d2, policy)
	require.NoError(t, err)
...
func TestCompression_AggregateExhaustion_PreservesOriginals(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		...
		_, err := cs.ReserveCompression(context.Background(), p, fmt.Sprintf("t-%d", i), d, policy)
		require.NoError(t, err)
	}
	...
	res1, err := cs2.ReserveCompression(context.Background(), p1, "t1", d1, policy)
	require.NoError(t, err)
	...
	res2, err := cs2.ReserveCompression(context.Background(), p2, "t2", d2, policy)
	require.NoError(t, err)
...
func TestCompression_DeleteReappend_FreshState(t *testing.T) {
	...
	res2, err := cs.ReserveCompression(context.Background(), p, "t1", d2, policy)
	require.NoError(t, err)
...
func TestCompression_FIFOEviction_ClearsOptionalAndPreservesCounters(t *testing.T) {
	...
	for i := 0; i < 2; i++ {
		...
		res, err := cs.ReserveCompression(context.Background(), p, id, d, policy)
		require.NoError(t, err)
...
func TestCompression_Replacement_NoDrift(t *testing.T) {
	...
	d := appendArtifactForCompression(t, cs, p, "t1", 32)
	// v1
	res1, err := cs.ReserveCompression(context.Background(), p, "t1", d, "v1")
	require.NoError(t, err)
	...
	res2, err := cs.ReserveCompression(context.Background(), p, "t1", d, "v2")
...
func TestCompression_Reserve_SameRevisionDuplicateRejected(t *testing.T) {
	...
	d := appendArtifactForCompression(t, cs, p, "t1", 32)
	res, err := cs.ReserveCompression(context.Background(), p, "t1", d, "v1")
	require.NoError(t, err)
	...
	require.NoError(t, cs.AttachSurrogate(context.Background(), p, "t1", res, auxiliary.JobID("job1"), sur))
	_, err = cs.ReserveCompression(context.Background(), p, "t1", d, "v1")
	require.Error(t, err)
...
func TestCompression_Reserve_WhilePendingRejected(t *testing.T) {
	...
	require.Error(t, err)
	assert.True(t, reasoningpreservation.IsConflictError(err))
	_, err = cs.ReserveCompression(context.Background(), p, "t1", d, "v2")
	require.Error(t, err)
	assert.True(t, reasoningpreservation.IsConflictError(err))

...
type recordingJSONMatcher struct {
	scanTokens   []string
	redactTokens []string
	scanHits     map[string][]secretguard.Finding
	redactHits   map[string][]secretguard.Finding
	redactions   map[string]string
}

func newRecordingJSONMatcher() *recordingJSONMatcher {
	return &recordingJSONMatcher{
		scanHits:   make(map[string][]secretguard.Finding),
		redactHits: make(map[string][]secretguard.Finding),
		redactions: make(map[string]string),
	}
}

func (m *recordingJSONMatcher) ScanBytes(ctx context.Context, input []byte) ([]secretguard.Finding, error) {
	return m.ScanString(ctx, string(input))
}

func (m *recordingJSONMatcher) ScanString(_ context.Context, input string) ([]secretguard.Finding, error) {
	m.scanTokens = append(m.scanTokens, input)
	if findings, ok := m.scanHits[input]; ok {
		return append([]secretguard.Finding(nil), findings...), nil
	}
	return nil, nil
}

func (m *recordingJSONMatcher) RedactBytes(ctx context.Context, input []byte) ([]byte, []secretguard.Finding, error) {
	redacted, findings, err := m.RedactString(ctx, string(input))
	return []byte(redacted), findings, err
}

func (m *recordingJSONMatcher) RedactString(_ context.Context, input string) (string, []secretguard.Finding, error) {
	m.redactTokens = append(m.redactTokens, input)
	if redacted, ok := m.redactions[input]; ok {
		if findings, ok := m.redactHits[input]; ok {
			return redacted, append([]secretguard.Finding(nil), findings...), nil
		}
		if findings, ok := m.scanHits[input]; ok {
			return redacted, append([]secretguard.Finding(nil), findings...), nil
		}
...
func findingForRef(ref string) []secretguard.Finding {
...
func TestGuard_JSONTokens_blockAndLog(t *testing.T) {
	...
	cases := []struct {
		...
		{name: "key_block", action: ActionBlock, raw: `{"blocked-key":"safe"}`, token: "blocked-key", ref: "KEY_SECRET", want: secretguard.OutcomeBlock},
		{name: "key_log", action: ActionLog, raw: `{"blocked-key":"safe"}`, token: "blocked-key", ref: "KEY_SECRET", want: secretguard.OutcomeLog},
		{name: "number_block", action: ActionBlock, raw: `{"n":7}`, token: "7", ref: "NUMBER_SECRET", want: secretguard.OutcomeBlock},
		{name: "number_log", action: ActionLog, raw: `{"n":7}`, token: "7", ref: "NUMBER_SECRET", want: secretguard.OutcomeLog},
		{name: "bool_block", action: ActionBlock, raw: `{"b":true}`, token: "true", ref: "BOOL_SECRET", want: secretguard.OutcomeBlock},
		{name: "bool_log", action: ActionLog, raw: `{"b":true}`, token: "true", ref: "BOOL_SECRET", want: secretguard.OutcomeLog},
		{name: "null_block", action: ActionBlock, raw: `{"n":null}`, token: "null", ref: "NULL_SECRET", want: secretguard.OutcomeBlock},
		{name: "null_log", action: ActionLog, raw: `{"n":null}`, token: "null", ref: "NULL_SECRET", want: secretguard.OutcomeLog},
	}

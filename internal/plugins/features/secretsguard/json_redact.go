...
func newUnsupportedJSONTokenError(findings []secretguard.Finding) error {
	return &unsupportedJSONTokenError{findings: append([]secretguard.Finding(nil), findings...)}
}
...
func redactJSONPayload(ctx context.Context, m secretguard.Matcher, raw []byte) ([]byte, []secretguard.Finding, error) {
	if len(raw) == 0 || m == nil {
...
// JSON uses ScanBytes.
func scanJSONPayload(ctx context.Context, m secretguard.Matcher, raw []byte) ([]secretguard.Finding, error) {
	if len(raw) == 0 || m == nil {
		return nil, nil
	}
	v, err := decodeJSONPreserveNumbers(raw)
...
func walkRedactJSON(ctx context.Context, m secretguard.Matcher, v *any) ([]secretguard.Finding, error) {
	if v == nil {
		return nil, nil
	}
	switch cur := (*v).(type) {
...
func walkScanJSON(ctx context.Context, m secretguard.Matcher, v any) ([]secretguard.Finding, error) {
	switch cur := v.(type) {
	case string:

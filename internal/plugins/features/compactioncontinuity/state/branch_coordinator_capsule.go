...
// CommitSource stores a bounded sanitized source snapshot for the post-open
// refresh path and the pre-open 7.12 preparation path (preview intent plus
// deterministic capsule before Open). It does not advance the capsule
// revision, but still compare-checks the current revision. Pre-open staged
// state is same-revision, TTL-bounded, and inert until a successful open
// binds the preview intent; failed opens must never bind intents, submit
// billable jobs, or commit release watermarks (§571).

	...
	for _, p := range planes {
		if strings.HasPrefix(p.typeExpr, "[]") {
			if p.hasIdentity {
				...
				buf.WriteString("\t\t} else if dst.values != nil {\n")
				fmt.Fprintf(buf, "\t\t\tif existingVal, exists := dst.values[%s.ID]; exists {\n", p.varName)
				fmt.Fprintf(buf, "\t\t\t\tif curTyped, ok := existingVal.(%s); ok {\n", p.typeExpr)
				buf.WriteString("\t\t\t\t\thadDestinationValue = len(curTyped) > 0\n")
				buf.WriteString("\t\t\t\t\tcurrent = cloneSlice(curTyped)\n")
				buf.WriteString("\t\t\t\t}\n")
				buf.WriteString("\t\t\t}\n")
				fmt.Fprintf(buf, "\t\t\texistingID = dst.identities[%s.ID]\n", p.varName)
				buf.WriteString("\t\t}\n")
				...
				buf.WriteString("\t\t}\n")
				buf.WriteString("\t\tif dst.values != nil {\n")
				fmt.Fprintf(buf, "\t\t\tdst.values[%s.ID] = clonedCombined\n", p.varName)
				buf.WriteString("\t\t}\n")
				buf.WriteString("\t\tif dst.identities != nil {\n")
				fmt.Fprintf(buf, "\t\t\tif finalHasID && finalID != \"\" {\n\t\t\t\tdst.identities[%s.ID] = finalID\n\t\t\t} else {\n\t\t\t\tdelete(dst.identities, %s.ID)\n\t\t\t}\n", p.varName, p.varName)
				buf.WriteString("\t\t}\n")
				...
				fmt.Fprintf(buf, "\t\t\t\treturn &AttributedError{\n\t\t\t\t\tPluginID: contributorID,\n\t\t\t\t\tPlaneID:  %s.ID,\n\t\t\t\t\tErr:      fmt.Errorf(\"%%w: %%w\", ErrInvalidContribution, err),\n\t\t\t\t}\n", p.varName)
				buf.WriteString("\t\t\t}\n")
				buf.WriteString("\t\t\tif (typed != nil || current != nil) && combined == nil {\n")
				fmt.Fprintf(buf, "\t\t\t\tcombined = make(%s, 0)\n", p.typeExpr)
				buf.WriteString("\t\t\t}\n")
				buf.WriteString("\t\t\tclonedCombined := cloneSlice(combined)\n")
				buf.WriteString("\t\t\tif dst.values != nil {\n")
			...
			buf.WriteString("\t\t\t}\n")
			buf.WriteString("\t\t} else {\n")
			buf.WriteString("\t\t\tvar current int\n")
			buf.WriteString("\t\t\tif dst.values != nil {\n")
			fmt.Fprintf(buf, "\t\t\t\tif existingVal, exists := dst.values[%s.ID]; exists {\n", p.varName)
			buf.WriteString("\t\t\t\t\tif curTyped, ok := existingVal.(int); ok {\n")
			buf.WriteString("\t\t\t\t\t\tcurrent = curTyped\n")
			buf.WriteString("\t\t\t\t\t}\n")
			buf.WriteString("\t\t\t\t}\n")
			buf.WriteString("\t\t\t}\n")
			fmt.Fprintf(buf, "\t\t\tcombined, err := %s.Combine(source, current, typed)\n", p.varName)
			buf.WriteString("\t\t\tif err != nil {\n")
			fmt.Fprintf(buf, "\t\t\t\treturn &AttributedError{\n\t\t\t\t\tPluginID: contributorID,\n\t\t\t\t\tPlaneID:  %s.ID,\n\t\t\t\t\tErr:      fmt.Errorf(\"%%w: %%w\", ErrInvalidContribution, err),\n\t\t\t\t}\n", p.varName)
			buf.WriteString("\t\t\t}\n")
			buf.WriteString("\t\t\tif dst.values != nil {\n")
			fmt.Fprintf(buf, "\t\t\t\tdst.values[%s.ID] = combined\n", p.varName)
			buf.WriteString("\t\t\t}\n")
			buf.WriteString("\t\t}\n")
			buf.WriteString("\t\tif dst.pluginIDs != nil {\n")
			fmt.Fprintf(buf, "\t\t\tdst.pluginIDs[%s.ID] = contributorID\n", p.varName)
			...
			buf.WriteString("\t\t}\n")
			fmt.Fprintf(buf, "\tif existingID, exists := dst.identities[%s.ID]; exists && existingID != \"\" {\n", p.varName)
			fmt.Fprintf(buf, "\t\treturn makeExclusiveConflictError(contributorID, %s.ID, %s.ExclusiveConflictError, existingID, srcID)\n", p.varName, p.varName)
			buf.WriteString("\t\t}\n")
			buf.WriteString("\tif dst.generated != nil {\n")
			...
			buf.WriteString("\t\t}\n")
			buf.WriteString("\tif dst.values != nil {\n")
			fmt.Fprintf(buf, "\t\tdst.values[%s.ID] = cloneValue(typed)\n", p.varName)
			buf.WriteString("\t\t}\n")
			buf.WriteString("\tif dst.identities != nil {\n")
			fmt.Fprintf(buf, "\t\tdst.identities[%s.ID] = srcID\n", p.varName)
			buf.WriteString("\t\t}\n")
			buf.WriteString("\tif dst.pluginIDs != nil {\n")
			fmt.Fprintf(buf, "\t\tdst.pluginIDs[%s.ID] = contributorID\n", p.varName)
			buf.WriteString("\t\t}\n")

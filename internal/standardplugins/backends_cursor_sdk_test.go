...
func TestStandardBackendBundle_registersCursorSDKLocalOnly(t *testing.T) {
	t.Parallel()
	...
	if !ok {
		t.Fatal("missing security profile for cursorsdk")
	}
	...
	if !found {
		t.Fatal("cursorsdk must be in StandardBackendBundle")
	}
}
...
func TestBackendCursorSDK_noCursorCLIACPRegression(t *testing.T) {
	t.Parallel()
...
`
	...
	_, err := backendCursorCLIACP(root, nil)
	if err == nil {
		t.Fatal("expected cursorcliacp to reject agy_binary")
	}
	if !strings.Contains(err.Error(), "agy_binary") {
		t.Fatalf("error = %v, want agy_binary", err)
	}
}

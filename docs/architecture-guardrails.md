...
The current `internal/core` ceiling includes the usage-authority stabilization pass, follow-on review remediation (explicit monetary cost presence, unenforceable spend-cap candidate exclusion, multi-scope settlement aggregation, and zero-remaining output budget denial), and live model-registry inventory/runtime growth for issue #146 (HTTP OpenAI `/v1/models` DTO remains in `internal/stdhttp`). Prefer decomposing further core growth rather than absorbing it by another budget increase.

...
The current `internal/core` ceiling includes the usage-authority stabilization pass, follow-on review remediation (explicit monetary cost presence, unenforceable spend-cap candidate exclusion, multi-scope settlement aggregation, and zero-remaining output budget denial), live model-registry inventory/runtime for issue #146, #149 publish-time `/v1/models` JSON + refresh/fingerprint growth in `modelregistry` (HTTP handler remains in `internal/stdhttp`), issue #152 core work in `internal/core/toolcallrepair` / runtime finalization, and protocol-neutral `internal/core/jsonshape` Token preflight (shared by frontend `jsonguard`). Measured post-change non-test total is 53816 lines; the cap is 53900. Prefer decomposing further core growth rather than absorbing it by another budget increase.

...
The `internal/infra/runtimebundle` ceiling was raised to 6400 for dual-plane Phase 2.2–5.5 composition plus Phase 5 remediation (descriptor-bound registrations, terminal-work ownership/readiness, executable generation compile/publish/readiness, provider-removal validation, pending-drain binding). Prefer keeping Build orchestration thin; new registration validation belongs on the public facade when practical.

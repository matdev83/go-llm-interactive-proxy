...
The current `internal/core` ceiling includes the usage-authority stabilization pass, follow-on review remediation (explicit monetary cost presence, unenforceable spend-cap candidate exclusion, multi-scope settlement aggregation, and zero-remaining output budget denial), live model-registry inventory/runtime for issue #146, and #149 publish-time `/v1/models` JSON + refresh/fingerprint growth in `modelregistry` (HTTP handler remains in `internal/stdhttp`). Prefer decomposing further core growth rather than absorbing it by another budget increase.

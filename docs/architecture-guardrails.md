...
The `internal/infra/runtimebundle` ceiling was raised to 5850 for dual-plane Phase 2.2 descriptor-bound provider/rater registration composition plus Phase 4.4 terminal-work processor ownership/remediation (ProductionOptions/Built/buildTerminalWorkRuntime/TerminalWorkReadiness; measured ~5801). Prefer keeping Build orchestration thin; new registration validation belongs on the public facade when practical.

...
The `internal/infra/runtimebundle` ceiling was raised to 6150 for dual-plane Phase 2.2–4.5 composition (descriptor-bound registrations, terminal-work ownership/readiness, RequestRegistrations→AuthorityRequestEffectProvider merge, ProcessDue metrics observer; measured ~6105). Prefer keeping Build orchestration thin; new registration validation belongs on the public facade when practical.

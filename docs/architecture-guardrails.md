...
The current `internal/core` ceiling includes prior usage-authority / model-registry / tool-call-repair / jsonshape / secrets-guard growth plus cursor-sdk and dual-plane readiness follow-up (`metering/plane` pure helpers wired from runtime egress, operator usage evidence helpers wired into operator settle/egress with lastAuthorityUsage → seenEvents → empty UsageDelta shell precedence for req 1.5 / 2.9, AttemptClampPreviewer `PreviewAttempt` + `SkipEvidence` with `AttemptCoordinator.Preview` and a bounded runtime clamp-preview loop before freeze (V-15), and `execbackend.Backend.Close`). Accidental duplication in `metering_egress` was removed before raising the budget. Measured non-test total is 56820 lines; the cap is 57000 (~180 lines of headroom). Prefer decomposing further core growth rather than absorbing it by another budget increase.

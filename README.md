...
- **Public production facade** - `pkg/lipruntime` lets a closed Go module build the standard distribution with injected metering, authority, concurrency, snapshot, and observer implementations without importing `internal/` packages. `cmd/lipstd` remains the OSS CLI composition root.

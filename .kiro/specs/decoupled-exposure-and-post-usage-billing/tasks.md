...
- [ ] 0.1 Characterize current billing/admission behavior and record a deletion/LOC baseline.
...
- [ ] 0.2 Add failing SafetyMargin and financial/exposure-separation tests.
...
- [ ] 0.3 Add failing identity tests for multiple billable calls on one A-leg/session.
...
- [ ] 0.4 Add planned architecture ratchets for the end-state.
...
- [ ] 1.1 Implement BillingCallID and call/leg identity/fingerprint contracts.
...
- [ ] 1.2 Add Bun-backed independent terminal B-leg usage append.
...
- [ ] 1.3 Add call-closure append and complete-call storage join.
...
- [ ] 2.1 Implement the cheap pre-routing credit screen.
...
- [ ] 2.2 Refactor current pessimistic estimation into a quote-only post-route path.
...
- [ ] 2.3 Implement atomic Bun `CallExposure` admission.
...
- [ ] 2.4 Cut authoritative admission from monetary holds to cheap-screen + exposure admission.
...
- [ ] 3.1 Remove hold lookup from customer rating input.
...
- [ ] 3.2 Implement idempotent customer billing operations including exact-zero calls.
...
- [ ] 3.3 Atomically apply customer balance mutation and close exposure.
...
- [ ] 3.4 Cut the post-usage worker to complete-call customer settlement.
...
- [ ] 4.1 Extract provider-cost calculation from the all-or-nothing customer settlement contract.
...
- [ ] 4.2 Implement independent idempotent provider-cost operations.
...
- [ ] 4.3 Decouple provider-cost retry/state from customer processing.
...
- [ ] 4.4 Update reports for settled customer spend, open exposure and provider COGS as separate perspectives.
...
- [ ] 5.1 Replace `billingTurnCollector` evidence aggregation with direct terminal leg appends.
...
- [ ] 5.2 Remove billing-specific parallel barrier/sealed/TUR-builder state.
...
- [ ] 5.3 Remove runtime hold cleanup and abort-release branches.
...
- [ ] 5.4 Shrink `BillingRuntime`/host composition to final seams.
...
- [ ] 6.1 Separate journal-derived financial rebuild from exposure reconstruction.
...
- [ ] 6.2 Implement stale-exposure recovery requiring positive durable evidence.
...
- [ ] 6.3 Reconcile/migrate legacy open holds and prevent creation of new holds.
...
- [ ] 6.4 Delete retired hold/reserved/authorization-book code and schema ownership.
...
- [ ] 7.1 Activate runtime/import/symbol architecture guards.
...
- [ ] 7.2 Prove cross-dialect concurrency and crash/replay behavior.
...
- [ ] 7.3 Update steering, architecture and host-composition docs.
...
- [ ] 7.4 Run final shrinkage and full behavior certification.
  ...
  - _Validation: make quality-checks; make test-unit; make test; make test-race where supported_

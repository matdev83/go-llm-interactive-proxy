...
This revision was cross-checked against `main` at `40168ce1f3890a1c86c22e898be9d264d63ccd72` after PR #533. The audit found several contracts that the original draft did not model strongly enough: the current typed plane architecture, the legacy hook bus outside that plane set, decode QoS, deterministic request/economic identity, secure-session recorder/session-response carriers, unconditional metering checkpoint Call retention, route-override availability in standard continuity stores, frontend response state, OpenResponses `store=true` defaulting, and deep post-identity Call dependencies.
...
3. run low-allocation protocol semantic proof under that permit;
4. while the same permit is still held, run a side-effect-free core wire-eligibility assessment;
...
This keeps all expected optimization declines before `SecureSession.BeginTurn`, avoids duplicate lifecycle/accounting side effects, and avoids a fallback-induced second decode-admission decision.

## Boundary Context

- **In scope**: optional large-payload ingress capture; bounded-memory replay; streaming shared JSON validation; bounded protocol semantic proof and canonical identity digest; decode-QoS parity; side-effect-free pre-turn eligibility; typed extension/hook/non-plane authority classification; late-bound route-authority compatibility envelopes; same-wire backend proof; bounded model rewrite; secure-session/metering wire views; retry/failover replay; frontend response/session-carrier parity; gzip follow-up; cancellation/resource cleanup; metrics; differential conformance; allocation/load benchmarks; conservative rollout.
- **Out of scope**: native provider passthrough (#490); cross-protocol raw forwarding; weakening canonical validation; changing default request-size limits; changing route/failover semantics to manufacture eligibility; bypassing secret/DLP/guardrail/accounting/traffic authorities; arbitrary new content encodings; provider SDK types in core; broad capability-profile work owned by #495; expected post-commit canonical fallback in V1.
- **Boundary ownership**: frontend ingress owns capture, shared JSON validation, decode admission, protocol proof, exact canonical fallback decode, and frontend-only response state; core/runtime owns side-effect-free eligibility, request identity consumption, secure-session/B2BUA lifecycle, route authority, metering/accounting, attempts, and terminal ownership; backend plugins own exact/universal wire compatibility and HTTP construction; composition freezes generation-scoped eligibility summaries.
...
4. If wire consideration declines after client bytes were consumed but before wire commit, the replay source shall feed the canonical decoder exactly once and in order.
...
**Objective:** The feature must not create a second admission policy.
...
7. The profile proof shall include the same certified-subset `Call.Validate`/frontend normalization semantics needed before executor entry.
...
6. The summary shall be built at composition time without hot-path reflection, request-path map walks, or repeated type assertions.
...
#### Acceptance Criteria
1. After whole-body shared preflight, the frontend shall acquire the current byte-weighted `DecodeAdmission` permit and retain it through protocol semantic proof **and** the side-effect-free core assessment.
2. `AssessLargeBody` (or equivalent) shall perform no `BeginTurn`, A-leg creation/fetch, DB/store mutation, billing reservation, provider network I/O, or other durable/per-turn side effect.
...
8. A future late-fallback mechanism requires a separate design; it is not required for #503 V1.
...
#### Acceptance Criteria
1. Mere presence of `RouteOverrideReader` shall **not** automatically block V1: standard memory/Bun continuity stores expose this capability and runtime composition wires it when available.
...
3. For session route overrides, the envelope shall be derived from the same generation validator/known-backend/execution-composition policy that accepts override selectors. Because override model text is not necessarily a finite catalog, a backend whose wire compatibility depends on a finite candidate model may need to prove an `any accepted model`/equivalent universal model contract or make the envelope ineligible.
...
### Requirement 13: Content/Policy/Traffic Authorities Remain Authoritative
...
#### Acceptance Criteria
1. Wire facts shall carry the exact bounded client/session authority inputs that canonical `BeginTurn` receives (for example authoritative session ID, resume token, client-session/continuity hints when applicable), sourced through normal frontend/header/body precedence. Sensitive resume tokens never reach backends or telemetry.
2. Initial profiles may conservatively reject body-carried LIP session metadata while still supporting the authoritative LIP session/resume headers; any supported body metadata must match current precedence exactly.
3. Standard secure-session recording shall not be blocked merely because the recorder is always composed. The protocol proof shall produce a bounded **normalized client-turn shape** equivalent to `lipapi.NormalizedItems` for the certified subset (role/ordinal/content-part kinds and other recorder-required non-content facts).
4. If normalized item/part metadata exceeds configured wire metadata bounds, the request shall be canonical.
5. Wire execution shall call the existing recorder with an equivalent `ClientTurnRecordInput` built from the bounded shape; prompt text shall not be materialized solely for this recorder.
6. New-session `BeginTurn` response carriers—including authoritative session ID, A-leg ID, and the raw resume token returned for a new session—shall be returned to the frontend through a **sensitive response-carrier contract** and emitted with the same session-header semantics as canonical execution.
...
#### Acceptance Criteria
1. The wire path shall not call the current Call-cloning frontend/backend metering checkpoint helpers merely to satisfy existing APIs.
...
3. Wire checkpoint storage shall not retain a hidden full canonical Call. Widening/retry integrity shall instead use immutable source/proof digests plus the bounded rewrite/attempt plan.
4. When token accounting/preflight is disabled, wire checkpointing shall remain fully usable without tokenization.
5. When configured accounting/context/billing requires input-token counting, the request is eligible only if an exact wire counting contract exists for the profile/backend/source (for example a streaming/provider-specific raw count path). Raw body byte count is never substituted for tokens.
...
### Requirement 16: Deterministic Request/Economic Identity Parity
...
4. Helpers may derive the current stable Call ID/token/timestamp from that digest so wire trace/economic identity and client-visible deterministic fallback fields remain path-stable.
5. A raw-body-only hash that differs from canonical `StableCallID` is not sufficient for economic identity, because path/config changes could produce different restart-stable metering fact IDs for the same request.
6. Caller/provider supplied explicit IDs retain current precedence; wire execution shall not overwrite them with a new hash scheme.
...
4. OpenResponses create is not eligible merely because `previous_response_id` is absent: current decoder defaults `store=true`. Its first certification requires **explicit `store:false`**, absent `previous_response_id`, no compaction, and no WebSocket unless continuation/storage parity is separately implemented.
...
7. Frontend-specific state stays at the frontend boundary; core SDK types do not import provider/frontend state.
...
3. Every entry is classified as: exact bounded wire fact/view; wire source/digest contract; response-only; or pre-assessment canonical blocker.
4. Standard always-composed components (secure-session recorder and metering checkpoint) require an explicit wire-native path rather than being silently classified as permanent blockers.
...
### Requirement 20: Replay-Spool Resource Safety and Confidentiality
**Objective:** Pre-commit replay stays bounded, deterministic, and safe for prompt data.
...
6. `Source.Close` is idempotent and shall not deadlock waiting for leaked readers; root close marks deletion pending and final removal occurs when tracked readers close. No cleanup goroutine is required.
7. File create/write/read/remove failures, cancellation, timeout, and Windows open-file deletion behavior receive injected/state tests.
8. Operator docs state that spool files can contain plaintext prompt data and describe `spool_dir`, volume/filesystem protection, and lifetime.
...
4. Verify decode permits are never held while waiting for client upload; measure the short additional hold across side-effect-free assessment.
5. Publish an eligibility matrix for: typed extension planes; legacy hook chains; frontend/core traffic; standard secure-session recorder; metering; accounting/billing; route override with homogeneous vs heterogeneous backend generations; sequential/fallback/race selectors; each certified frontend/backend lane.
6. At least one normal secure-session + metering production-like composition shall reach wire execution. A design that compiles but is blocked by standard always-on components is not complete.
7. Performance claims shall state that V1 still receives/validates the full request before provider open; the principal gain is heap/GC and avoided canonical object/marshal work, not early TTFT.

### Requirement 22: Conservative Rollout, Diagnostics, and Architecture Ratchets
**Objective:** Operators can enable, inspect, and revert the optimization safely.
...
4. Architecture tests prevent provider-name switches in core, provider types in request-body SDK contracts, duplicate typed-plane classification systems, unclassified request authorities, fake Calls, expected fallback after wire commit, and protocol proof outside decode admission.
5. Full QA/race/static checks, canonical characterization suites, identity-digest differential tests, and protocol differential tests pass before any production profile advertises wire support.
6. The spec/design shall be revalidated if main materially changes in frontend decode/admission, secure-session lifecycle, metering/accounting, extension/hook composition, routing authority, backend contracts, or response encoding before implementation begins.

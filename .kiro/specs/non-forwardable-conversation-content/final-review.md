...
| 13.17 | Final validation runs formatting/vet/arch, deterministic unit suites, SQLite/PostgreSQL (gated), backend-family sentinels, targeted `go test -race` | `make quality-checks` PASS, `go test ./...` PASS (cached; targeted `go test -count=1 ./internal/core/conversationview/... ./internal/core/b2bua/... ./internal/core/continuity/bunstore/... ./internal/core/runtime/...` PASS), `parity tcks` PASS, `bunstore` SQLite PASS, `PostgresContract` SKIP (no DSN), `race-check.ps1` SKIP on Windows | PASS (with env-gated SKIPs recorded) |
...
* PostgreSQL gated: `go test -tags=integration -run TestConversationView_PostgresContract ./internal/core/continuity/bunstore` **SKIP** — `set LIP_REQUIRE_POSTGRES=1 and LIP_TEST_POSTGRES_DSN to run` (no DSN in this worktree/CI stub). Same for `TestIntegration_Postgres_WriterLateReader_NoStaleCache` (uses `testkit.SkipUnlessPostgres`). Expected gated skip recorded; Linux CI with DSN runs full parity per `requirements.md` 13.8.
...
* PostgreSQL: skipped locally (no `LIP_TEST_POSTGRES_*`); validated in dedicated PG environment via `bunstore` `integration` tag and `test-authority-postgres` where DSN is provisioned.

...
- [ ] **1.** Capture the stage-four seam map, migration guardrails, and proof criteria in project docs before code changes expand the runtime surface
...
- [ ] **1.1** Add RED regression scaffolding for stage ordering, privilege visibility, proof-plugin exit criteria, and runtime-snapshot stability
...
- [ ] **2.** Replace hook-only feature assembly with a typed, versionable `FeatureBundle` contract in the stable SDK
...
- [ ] **2.1** Implement brownfield compatibility adapters so current hook-only feature plugins can register through the new bundle composition path
...
- [ ] **3.** Define the stage registry and typed stage descriptors for the extension pipeline
...
- [ ] **3.1** Implement deterministic intra-stage ordering and the per-stage failure matrix
...
- [ ] **3.2** Expose stage occupancy, ordering, failure mode, and privilege metadata through diagnostics/inventory surfaces
...
- [ ] **4.** Add stable plugin-facing context views for principal, session, attempt, workspace, and annotations
...
- [ ] **4.1** Define narrow service facades for plugin state, auxiliary requests, workspace access, and traffic observation/capture
  ...
  - _Design: **§2**, **§7**, **§8**, **§10**_
  - _Boundary: `pkg/lipsdk/state`, `pkg/lipsdk/aux`, `pkg/lipsdk/workspace`, `pkg/lipsdk/traffic`_
  - _Depends: **4**_
  - _Validation: `go test ./pkg/lipsdk/...`_

- [ ] **4.2** Implement immutable runtime execution snapshots and bind each request to one stable snapshot for its lifetime
...
- [ ] **4.3** Add deterministic fakes/builders for context views, service facades, and runtime-snapshot publication
...
- [ ] **5.** Add the transport-auth seam to `stdhttp` and propagate a canonical principal view into the execution pipeline
...
- [ ] **5.1** Implement the session-open stage and workspace-resolution seam
...
- [ ] **6.** Implement the plugin-scoped state service with namespace, scope, TTL, and deterministic test seams
...
- [ ] **6.1** Add expiry-aware inspection semantics and prove state-backed behavior with deterministic tests
...
- [ ] **7.** Implement the request-transform stage and integrate it with existing submit/request-part hooks
...
- [ ] **7.1** Add the tool catalog filter stage before backend translation and tool-choice reconciliation
...
- [ ] **7.2** Extend tool-event reaction contracts without leaking provider SDK types into core or SDK packages
...
- [ ] **8.** Implement the auxiliary-request service with lineage, role support, loop guards, and request-lifetime safety rules
  ...
  - _Design: **§6**, **§7**, **§17**_
  - _Boundary: `pkg/lipsdk/aux`, `internal/core/auxreq`, `internal/core/runtime`_
  - _Depends: **4.1**, **6**_
  - _Validation: `go test ./pkg/lipsdk/aux/... ./internal/core/auxreq/... ./internal/core/runtime/...`_

- [ ] **8.1** Add route-role and route-hint plumbing while keeping routing authoritative in core
...
- [ ] **9.** Implement the completion-gate seam, typed decision model, and bounded buffering behavior
...
- [ ] **9.1** Add completion-gate regression tests for overflow, no-post-output replacement, and auxiliary-influenced final decisions
...
- [ ] **10.** Implement traffic observers, privileged raw capture sinks, and redaction-stage plumbing across all four traffic legs
...
- [ ] **10.1** Prove the per-leg observation pipeline contract with explicit raw-capture, redaction, and structured-observer tests
...
- [ ] **11.** Implement the minimum proof-plugin set required to show the new seams are extension-complete
...
- [ ] **11.1** Add `ref-autoappend-file` (or equivalent) to prove session-open plus submit/request shaping behavior
  - _Requirements: **R5**, **R12**, **R16**_
  - _Design: **§3**, **§5**, **§19**_
  - _Boundary: `internal/plugins/features`, `internal/testkit`_
  - _Depends: **5.1**, **7**_

- [ ] **11.2** Add `ref-tool-policy` (or equivalent) to prove tool catalog filtering plus tool-event enforcement
  - _Requirements: **R9**, **R16**_
  - _Design: **§4**, **§19**_
  - _Boundary: `internal/plugins/features`, `internal/testkit`_
  - _Depends: **7.2**_

- [ ] **11.3** Add `ref-workspace-guard` (or equivalent) to prove workspace resolution plus state-backed safety behavior
  - _Requirements: **R5**, **R6**, **R16**_
  - _Design: **§8**, **§9**, **§19**_
  - _Boundary: `internal/plugins/features`, `internal/testkit`_
  - _Depends: **5.1**, **6.1**_

- [ ] **11.4** Add `ref-traffic-transcript` (or equivalent) to prove four-leg observation and redacted export behavior
  - _Requirements: **R10**, **R11**, **R16**_
  - _Design: **§10**, **§11**, **§19**_
  - _Boundary: `internal/plugins/features`, `internal/testkit`_
  - _Depends: **10.1**_

- [ ] **11.5** Add `ref-verifier-stub` (or equivalent) to prove completion gating plus auxiliary-request replacement flow
...
- [ ] **12.** Publish extension-platform authoring guidance, migration notes, and seam-selection rules for future feature work
...
- [ ] **12.1** Add architecture tests and regression gates that prevent backsliding into core/plugin coupling
...
- [ ] **12.2** Add runtime-snapshot stability and inventory regression checks as final stage gates

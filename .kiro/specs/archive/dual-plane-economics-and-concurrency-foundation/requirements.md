...
- **In scope**: correction of current accounting semantics; immutable metering checkpoints; explicit economic perspective, lifecycle scope, and metering basis; idempotent usage facts; public enterprise injection contracts; request- and attempt-level authority orchestration; correct routing-attempt accounting; monetary presence and arithmetic hardening; dynamic versioned authority sources; distributed concurrent-request leases; durable metering and authority evidence; runtime and control-plane integration; transaction-pooled PostgreSQL compatibility for dual-plane durable stores without session affinity.
- **OSS core responsibility**: protocol-neutral metering facts, lifecycle timing, generic authority ports and coordination, safe scope attribution, a reusable concurrency authority, compatibility adapters for existing usage authority, protected backend query/evidence surfaces, and pooler-safe PostgreSQL connection/schema modes for metering, usage-authority, and concurrency-lease stores.
...
- **Out of scope**: frontend UI, web administration screens, customer portal, payment processors, invoice rendering, SSO/SAML/SCIM implementation, security/content policy engines, compression algorithms themselves, cloud marketplace packaging, and proprietary connection-pooler products beyond consuming standard PostgreSQL transaction-pooling endpoints.
- **Revalidation triggers**: frontend decode timing, request mutation stages, backend-open timing, stream mutation and completion gates, B2BUA attempt lineage, cancellation/finalization, parallel racing, token component semantics, price calculation, usage authority store transactions, runtime composition, control-plane schemas, extension-platform import boundaries, PostgreSQL connection-mode/schema-mode contracts, shared pool ownership, and pooler-safe store SQL.

## Amendment History

- **2026-07-15 — Transaction-pooled PostgreSQL compatibility (additive)**: Requirement 18 and related boundary/revalidation notes incorporate the approved six-phase execution plan for pooled runtime support without session affinity, including admin/runtime DSN separation, composition-owned pools, verify-only schema, CI PgBouncer gates, and parallel pooled proofs. Prior Requirements 1–17 remain in force; this amendment does not rewrite them.
...
### Requirement 18: Transaction-Pooled PostgreSQL Compatibility
**Objective:** As an operator deploying dual-plane durable stores against PostgreSQL transaction pooling (for example PgBouncer transaction mode or a managed pooled endpoint), I want migration/admin and runtime paths separated, runtime DML free of session affinity, and release gates that prove pooled behavior under normal parallelism, so heavily concurrent production traffic is not limited to direct/session-preserving connections.

#### Acceptance Criteria
18.1. WHEN dual-plane durable stores run against PostgreSQL, THE proxy SHALL support a runtime connection path with `connection_mode=transaction_pool` that requires no session affinity or sticky backend connections across transactions.
18.2. THE migration/admin path SHALL use a direct PostgreSQL role/DSN and SHALL own DDL and schema migration; THE runtime path SHALL support `direct` or `transaction_pool`, SHALL execute DML only under `schema_mode=verify_only` when pooled, and SHALL NOT share the runtime transaction-pool role for schema-mutating work by default.
18.3. PostgreSQL configuration SHALL expose explicit `connection_mode: direct|transaction_pool` and `schema_mode: auto_migrate|verify_only`; unknown values SHALL fail validation with actionable errors.
18.4. THE combination `connection_mode=transaction_pool` with `schema_mode=auto_migrate` SHALL be rejected at validation/startup (fail closed); pooled runtime SHALL require `verify_only`.
18.5. WHILE preserving compatibility for existing deployments, THE default SHALL remain `direct` + `auto_migrate` until an explicit rollout changes generated config; enabling `transaction_pool` SHALL require `verify_only` and SHALL NOT silently break existing direct deployments.
18.6. THE composition root SHALL own one bounded runtime pool per distinct sanitized runtime DSN + pool config; authority, lease, and journal adapters SHALL receive injected handles; THERE SHALL be no globals, DI container, service locator, or default per-store duplicate pools for the same key; EACH shared pool SHALL close exactly once; partial build failure SHALL clean up exactly once.
18.7. Runtime dual-plane SQL (authority, lease, metering journal, readiness, and query paths) SHALL NOT rely on `SET search_path`, session GUCs, temporary tables, SQL PREPARE/DEALLOCATE session affinity, session advisory locks, connection-local state, or multi-statement connection affinity outside a single explicit transaction.
18.8. Runtime multi-statement mutations SHALL stay on one transaction handle; correctness SHALL use targeted row locks/CAS only; THERE SHALL be no process-wide mutex across database I/O for unrelated keys (preserving Requirements 9.8, 9.9, 16.1, and 16.2).
18.9. WHEN schema mode is `verify_only`, startup/runtime SHALL verify required dual-plane schema presence and version/compatibility and SHALL NEVER mutate schema objects (no CREATE/ALTER/DROP and no migration runner on the runtime role). Empty or outdated schema SHALL fail closed with an actionable error.
18.10. THE distribution SHALL provide an explicit migration command of the form `lipstd migrate --components usage-authority,concurrency,metering` (exact flag spelling may follow repo CLI conventions) that applies reviewed versioned migrations via a direct admin DSN from `LIP_MIGRATION_POSTGRES_DSN` (not a CLI secret argument), uses a bounded context, returns nonzero on partial failure, and does not start the serving runtime.
18.11. Pool acquire failures, context cancellation while waiting for a connection, and pool exhaustion SHALL surface as classified infrastructure outcomes that are bounded, readiness-visible, and observable with bounded-cardinality metrics; they SHALL NOT be silently converted into capacity-exhaustion denials for unrelated principals (preserving Requirement 15.4).
18.12. WHERE serialization conflicts are evidence-driven, THE runtime MAY apply only bounded whole-transaction retry for SQLSTATE `40001`/`40P01`, remaining context-aware, never retrying deterministic capacity denial, preserving idempotency keys, and emitting bounded retry metrics by stable SQLSTATE class.
18.13. Production code SHALL NOT infer Neon or any provider by `-pooler` hostname substring or rewrite DSNs/providers automatically; connection and schema behavior SHALL be explicit configuration only.
18.14. Release gates SHALL separately prove (a) migration/admin compatibility on a direct endpoint, (b) direct runtime DML contracts, and (c) transaction-pooled runtime DML contracts. Direct-only or migration-only green SHALL NOT satisfy the pooled runtime gate. Aggregate dual-plane PostgreSQL green SHALL include both direct and pooled runtime proofs.
18.15. THE test harness SHALL define two explicit environment variables: `LIP_TEST_POSTGRES_ADMIN_DSN` for direct admin/bootstrap/cleanup/migration, and `LIP_TEST_POSTGRES_DSN` for transaction-pooled runtime DML. `LIP_MANAGED_POSTGRES_DSN` MAY remain only as a documented legacy alias. Make targets SHALL include `test-postgres-migrations`, `test-authority-postgres-direct`, `test-authority-postgres-pooled`, and aggregate `test-authority-postgres`.
18.16. Pooler-targeted contract tests SHALL NOT use session `search_path` for isolation. Isolation SHALL use globally unique store/fact/lease/source IDs plus targeted cleanup by `store_id` from the admin path, and/or fully qualified/admin-prepared objects. Pooled runtime proofs SHALL run with normal `-parallel=8`; `-parallel=1` SHALL NOT be an acceptable pooled runtime correctness proof. Aggregate targets SHALL NOT force `-parallel=1` for pooled DML; serial execution is allowed only for migration bootstrap where documented.
18.17. CI SHALL prove pooled runtime using local PostgreSQL plus PgBouncer in transaction mode (not an external managed service as the required gate). Operator metrics for pool open/idle/in-use/waits/wait duration, transaction latency by bounded store/operation, retry counts by stable SQLSTATE class, timeout/cancel, schema version/readiness, and connection mode SHALL use bounded cardinality (no high-cardinality labels).
18.18. Enabling `transaction_pool` SHALL NOT weaken Requirements 9.9, 10.2, 15.4, 16.1, 16.2, 16.3, 16.5, 16.6, 16.7, or 17.9; existing direct-connection PostgreSQL proofs remain valid for `connection_mode=direct`.

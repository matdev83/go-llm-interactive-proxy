...
2. Optional non-profile providers shall use the repository's external connector architecture unless an already-existing in-process family adapter is the documented architecture owner for the shared behavior.
...
5. Only configured provider instances shall create runtime backend/inventory resources according to the existing generation lifecycle.

...
- [x] 0.1 Resolve and verify the predecessor implementation dependency.
...
- [x] 0.2 Materialize the canonical widened production simplification baseline.
...
- [x] 0.3 Inventory every live legacy TUR/LUR/reserved/auth-book/money-authority/outbox consumer.
...
- [x] 0.4 Add planned architecture/LOC ratchets in non-activated state.
...
- [x] 1.1 Add differential tests between legacy bridge and native current-record rating.
...
- [x] 1.2 Implement native `RateCustomerCall` over current call/leg records.
...
- [x] 1.3 Cut customer resolver/worker to native rating only.
...
- [x] 1.4 Delete legacy TUR/LUR domain rating bridge and activate bridge-forbid ratchet.
...
- [x] 2.1 Add RED process-local spool durability/completeness/restart/idempotency/capacity contracts.
...
- [x] 2.2 Implement process-owned spool and one flusher.
...
- [x] 2.3 Cut runtime terminal handoff to `TerminalUsageSink`.
...
- [x] 2.4 Drain and delete direct+fallback central append layering.
...
- [x] 3.1 Add RED concurrency tests showing current provider COGS/customer admission serialization.
...
- [x] 3.2 Implement the single provider-independent journal ordering contract.
...
- [x] 3.3 Cut `ApplyProviderCost` off customer account locking/version mutation.
...
- [x] 3.4 Update reports/reconciliation for the two explicit ordering domains.

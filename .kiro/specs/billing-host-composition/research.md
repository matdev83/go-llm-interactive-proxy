...
| 2 | In-tree composer into existing host builder; single journal authority; no second host | `ProductionOptions` wiring already overwrites handoff/reports from store; worker start at Publish | **Missing**: helper that opens store, builds adapter, fills identity+catalog+resolver, returns `ProductionOptions` / documents the recipe |

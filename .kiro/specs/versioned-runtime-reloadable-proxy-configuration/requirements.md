...
Go-LIP is intended to serve large numbers of long-lived and concurrent LLM requests continuously. Its current standard runtime is assembled once at process startup: configuration is loaded, backends and feature chains are constructed, one HTTP handler graph is mounted, and the process serves that graph until shutdown. Any operational configuration change therefore requires a process restart, which can terminate or disrupt active streams and removes the ability to introduce a newly configured backend instance without a maintenance window.
...
- **Revalidation triggers:** changes to `runtimebundle.Build` ownership, plugin lifecycle contracts, `execbackend.Backend`, frontend mount contracts, model registry publication, secure-session or continuity stores, terminal-work provider ownership, routing/affinity/health state, `http.Server` setup, tracing/metrics bootstrap, or backend plugin discovery/trust policy.
...
### Requirement 2: Bounded, Strict, and Deterministic Configuration Loading

**Objective:** As an operator, I want every candidate configuration loaded through a strict and deterministic pipeline, so that malformed, partial, or ambiguous input can never become active.

#### Acceptance Criteria

2.1. When a reload begins, the configuration source shall read one bounded byte snapshot from the fixed startup path and shall reject input exceeding the configured startup-fixed limit.
2.2. If the source is missing, unreadable, empty, or whitespace-only, then the reload shall fail without replacing the active generation.
...
2.6. The effective candidate shall run all applicable core validation, access/auth posture validation, routing alias validation, custom-compatible prefix validation, feature composition validation, frontend validation, backend-factory validation, and runtime assembly validation before publication.
2.7. `check-config` and reload shall use the same effective validation rules; inspect-only validation shall not report a candidate valid when full candidate construction would deterministically reject it.
...
2.9. The source-read and decode path shall support operator use of atomic file replacement, and a partially written in-place file shall fail safely rather than displacing the active generation.
2.10. Validation errors shall identify safe field paths, plugin instance IDs, and bounded reason categories without including raw secrets, opaque configuration values, credentials, prompt data, or full file contents.
...
3.2. If any read, decode, normalization, validation, diff, construction, preparation, or readiness stage fails, then the active generation shall remain unchanged.
...
3.10. Each trigger shall produce one terminal result classified as published, no-op, invalid, restart-required, busy, retention-blocked, canceled, preparation-failed, or internal-failed.
...
4.1. A published runtime generation shall bind one normalized configuration, executor, backend instance set, routing/default-route/alias policy, capability view, frontend handler graph, transport-auth policy, feature/hook surface, model-registry runtime, request limits, and generation-owned resources.
...
4.3. Configuration values, maps, slices, route tables, hook chains, backend maps, and model snapshots owned by a published generation shall be treated as immutable.
...
4.8. Public status and diagnostics shall expose only bounded, non-secret generation metadata and shall never expose private effective digests or serialized configuration.
4.9. When a generation is published, all components reachable through its handler and executor shall belong to that generation or to an explicitly shared process service; no unclassified ownership shall remain.
...
5.9. Publication shall perform no configuration I/O, backend construction, lifecycle start, model fetch, or old-generation drain wait while holding the active-generation commit operation.
...
6.9. Generation compilation shall receive shared services through explicit construction inputs and shall not reach them through globals or a service locator.
...
10.1. A generation shall have explicit preparing, active, retiring, quiesced, drained, closing, closed, and failed lifecycle states or equivalent observable transitions.
...
10.5. When a generation retires, the runtime may quiesce refresh loops or other admission-independent background workers that are no longer needed, while preserving immutable state and resources required by pinned work.
...
10.12. Cleanup failures after drain shall be reported and retried only through an explicitly owned cleanup policy; they shall not roll back or corrupt the already active newer generation.
...
12.5. In explicit single-user local mode, a loopback-only management endpoint may use the documented local trust posture; multi-user mode or any non-loopback management bind shall require a dedicated strong startup-fixed authentication secret or injected administrator authenticator.
...
12.7. Unauthorized, malformed-method, wrong-content-type, oversized, and cross-origin browser-style requests shall be rejected before invoking the coordinator.
12.8. The reload response shall use stable safe result categories and appropriate HTTP status codes for published/no-op, invalid, restart-required, busy, retention-blocked, canceled, and internal/preparation failure outcomes.
...
12.11. Management listener address, paths, authentication mode, authentication material source, body limits, and reload timeout shall be startup-only fields.
...
13.5. If post-publication retirement or cleanup of the prior generation fails, then the new active generation shall remain active and the cleanup failure shall be surfaced separately.
...
14.1. The proxy shall retain bounded status for the active generation, current attempt, most recent successful publication, most recent failed/no-op attempt, and retained generations.
...
14.5. Metrics shall expose active/retired generation counts and aggregate pinned-reference posture without using generation ID, backend ID, model ID, file path, or user-controlled text as a metric label.
14.6. Where tracing is enabled, a reload attempt shall create process-owned spans for source read, validation, compilation, preparation, publication, and cleanup without replacing the global tracer provider.
...
14.8. The management status surface shall distinguish an invalid candidate, restart-required change, retention blockage, cleanup failure, and healthy active generation.
...
15.4. Publication shall be a bounded atomic commit independent of candidate size and old-generation drain duration.
15.5. The coordinator shall bound source bytes, decode complexity, candidate build time, remote optional probes, retained generations, status history, and concurrent reload work.
...
15.8. Race-detector and goleak tests shall cover trigger concurrency, publication/acquire races, request/async pin transfer, shutdown races, lifecycle rollback, and retirement cleanup.
...
16.2. The public facade shall expose explicit reload and safe status operations without exposing `internal/...` types, mutable configuration objects, provider SDK types, or raw handler/runtimebundle internals.
16.3. Existing startup, `check-config`, routes, inventory, and normal serving behavior shall remain compatible when runtime reload configuration is omitted or left at defaults.
...
16.7. Interfaces, lifecycle contracts, reloadability classifiers, API contracts, and failing concurrency/no-drop scenarios shall be written before production implementation.
16.8. Deterministic tests shall require no live provider, real credential, mutable external service, or filesystem watcher.
16.9. Release evidence shall include focused unit and composed tests, architecture gates, race and leak runs, fuzz smoke for source/decode/diff inputs, parity checks, full QA, and a bounded reload soak.
16.10. Operator documentation shall describe explicit triggers, atomic file replacement, the reloadability matrix, restart-required errors, management security, version/status semantics, failure recovery, retained-generation pressure, and the absolute absence of automatic file watching.

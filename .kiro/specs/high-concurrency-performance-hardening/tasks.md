...
- [ ] 1.2 Implement the internal scenario, result, and certification-gate contracts
  ...
  - Fingerprint backend execution path and cancellation schedule independently: built-in/standard versus negotiated executable plugin, launch-in-flight, sibling count, graceful acknowledgement, deadline/forced close, explicit cancel/terminal offsets, and upstream terminal outcome. Include a stable reference-backend script ID plus immutable version or content digest.
  ...
  - Model proxy, each managed connector child, and a first-class aggregate process-tree resource scope separately; sum aligned member samples/counter deltas exactly once, derive peak gauges from maximum aligned sums rather than sums of independent peaks, prefer cgroup/process-tree or aligned PSS/USS for unique physical memory, label summed RSS overlap, and mark incomplete aggregates unavailable. Derive fixed-per-stream versus request-start/per-event/terminal/cleanup cost rather than one aggregate average.
  - Add versioned `CertificationGateProfile` types with frozen warm-up/steady duration, valid repetition count, correctness/durability/error gates, applicable sourced latency/resource/fixed-stream/request-start/event-rate/terminal-burst/cleanup/DB/scheduler gates, memory-growth methods, and gate-profile fingerprinting.
  - Unit-test serialization, stable fingerprints, zero-vs-unavailable metrics, HTTP/1.1/HTTP2 stream/connection validation, upstream success/failure/timing/script-version fingerprint inequality, non-coincident process-peak aggregation, invalid-correctness classification, limiter classification, and per-phase gate PASS/NO-GO/missing-evidence behavior.
...
- [ ] 1.3 Implement the standalone load runner and scripted-upstream orchestration
  ...
  - Reuse reference clients for actual frontend wire behavior and control the reference upstream's TTFT/event cadence/event bytes/failure/finish schedule deterministically under a stable script ID and immutable version/content digest.
  ...
  - Drive equivalent HOLD/DELTA streams through the built-in/standard and negotiated executable-plugin paths where supported, and drive launch-in-flight, one/many sibling, graceful, forced-close, and distinct upstream-success/upstream-failure race schedules using existing #446 contracts rather than duplicating them.
...
- [ ] 1.4 Integrate profiling/resource/database evidence collection and privacy validation
  ...
  - Attribute DB reads/writes/queries/transactions to request-start, stream-event, terminal, and cleanup phases and assign exactly one canonical classification: authoritative synchronous work (including mandatory stream/event records), reusable/versioned read work, best-effort projection, or idempotent terminal work. Collect proxy/connector process-tree goroutine/stack/channel/buffer and scheduler/runnable/blocking evidence.
...
- [ ] 1.5 Freeze scenario definitions, gate profiles, and benchmark-scratch procedure
  ...
  - Freeze the applicable fixed-stream, request-start, event-rate allocation/GC, terminal-burst, cleanup, per-phase DB-operation, shared-contention, scheduler, and process-tree envelopes before their certification runs; define missing/unavailable/failing per-phase NO-GO behavior and record `GOMEMLIMIT`/`GOGC` as controlled environment values rather than hidden tuning.

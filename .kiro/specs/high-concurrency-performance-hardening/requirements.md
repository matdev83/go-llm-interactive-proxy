...
- **In scope**: deterministic load/traffic generation; performance evidence capture; logical-stream and transport-connection measurement; streaming timeout semantics; request-body admission placement; secure-session stream recording and persistence; secure-session/B2BUA lifecycle query amplification; bounded stream/evidence memory; auth event dispatch contention; B2BUA continuity-store contention/expiry; concurrency-authority lease-store contention; durable billing-spool completion throughput; canonical-call and per-event allocation/GC pressure; traffic-observation overhead; residual shared runtime/credential/state/affinity/lifecycle locks; hot-path database-operation classification; bounded cache/buffer/work reuse where proved; per-stream goroutine/channel/stack/scheduler cost across the proxy and managed connector processes; HTTP connection/transport tuning; resource cleanup; profiling/metrics needed to prove changes; final 1k/5k/10k logical-stream and soak certification.
- **Out of scope**: new LLM protocol semantics; selector/routing policy redesign; provider-specific behavior in core; new monetary/billing semantics; weakening secure-session mandatory durability/fail-closed policy; blanket removal of authoritative database operations; replacing the goroutine-per-stream model without evidence; generic worker-pool/actor frameworks; pooling lifecycle-bearing stream operations merely to reduce goroutine counts; Cartesian frontend×backend performance matrices; production-provider load testing; public SDK/config growth solely for benchmarks.
...
19. The disconnect/cancellation matrix and scenario fingerprints shall cover cancellation while a B-leg launch is in flight, one versus multiple sibling B-legs, graceful cancellation acknowledgement, deadline/forced-close fallback, and races with both upstream success and upstream failure. Each race fingerprint shall include the explicit cancellation/terminal schedule and a stable reference-backend script identifier plus version or content digest while the run proves exact terminal, usage, billing, and cleanup behavior.
...
### Requirement 19: Bound Cross-Cutting Per-Session Runtime Cost

**Objective:** As an operator targeting thousands to tens of thousands of simultaneous sessions, I want the proxy's fixed per-stream and rate-driven hot-path costs explicitly bounded and certified, so that database I/O, allocation/GC, shared state, and goroutine scheduling cannot become hidden multiplicative limits.

#### Acceptance Criteria

1. Before a cross-cutting runtime optimization is selected, the performance program shall report fixed cost per active logical stream separately from request-start, per-event, terminal, and cleanup cost, including RSS/live heap, stack bytes, goroutines, reserved/occupied channel or buffer capacity where observable, allocation bytes/counts, GC activity, scheduler/blocking evidence, shared-lock contention, and logical/physical database operations.
2. Where executable backend plugins are exercised, the cost report shall identify proxy-process, each managed connector-process, and aggregate process-tree resource use separately so that moving work across the IPC boundary cannot be reported as a proxy-wide reduction. Aggregate counters and gauges shall include every member exactly once over aligned intervals, define peak and shared-memory overlap semantics, and mark incomplete membership unavailable rather than zero.
3. At every attempted 1k/5k/10k tier, the system shall report the scaling slope and frozen sourced gates separately for each applicable fixed-stream, request-start, per-event, terminal-burst, and cleanup cost envelope, and shall not certify a tier merely because connections remain open when an applicable phase gate, resource headroom, latency, correctness, durability, or cleanup gate is missing or fails.
4. The implementation program shall classify database reads, writes, queries, and transactions on request-start, stream-event, terminal, and cleanup paths as exactly one of these canonical classes before selecting caching, batching, or asynchronous execution: authoritative synchronous work (including mandatory stream/event records), reusable/versioned read work, best-effort projection, or idempotent terminal work.
5. When repeated database reads are safely reusable, the implementation shall first reuse already-loaded or immutable versioned state where possible; any cache selected instead shall have explicit freshness, invalidation/versioning, entry/byte bounds, expiry, miss-stampede, failure, observability, and shutdown behavior.
6. Cached or asynchronously projected state shall not be used as stale authority for authentication, secure-session mandatory durability, concurrency/exposure admission, financial settlement, billing evidence, or another contract that requires current transactional truth.
7. A database write may leave the initiating request/event call stack only when the existing failure policy is best-effort or when a durability-preserving local acknowledgement contract is proved; every such queue/spool shall be bounded and shall define overload, retry, crash/restart, drain, cancellation, and shutdown semantics.
8. For material allocation sites, equivalent before/after evidence shall compare preallocation, ownership/copy elimination, streaming/incremental processing, and bounded object/buffer reuse as applicable, and shall retain only the smallest mechanism that measurably reduces allocation/GC cost without unacceptable secondary regressions.
9. Where temporary objects or buffers are reused across operations, the system shall enforce bounded retained capacity, discard oversized outliers, fully reset state, prevent sensitive or session-specific data from becoming observable by another session, and prove cleanup plus allocation benefit under representative concurrent load.
10. Mutable request, attempt, stream, and session state shall remain lexical/per-owner or generation-immutable unless cross-session sharing is required by a process-wide invariant; process-shared locks shall not cover external I/O, database calls, serialization, callbacks, or other unbounded work.
11. The number of goroutines and channels owned by one active stream/attempt shall be fixed and bounded independently of event count, and no event-cadence path shall spawn an unowned goroutine; each owner shall have cancellation, termination, join, panic/error, and post-cleanup verification.
12. A shared worker pool may be introduced only for bounded short-lived independent work with explicit concurrency, queue capacity, full behavior, fairness, cancellation, and shutdown contracts plus measured end-to-end benefit; it shall not silently take ownership of blocking stream receive, ordered emission, cancellation, or close lifecycles.
13. Runtime GC tuning such as `GOMEMLIMIT` or `GOGC` shall be recorded as an environment variable and tested as a controlled experiment when relevant, but shall not substitute for bounding live state, allocation churn, queues, buffers, database work, or goroutine ownership.
14. Final certification shall state the retained/rejected cache, async-write, reuse-pool, shared-state, and goroutine-topology decisions together with the measured per-phase cost envelope and the dominant remaining multiplicative limiter.

...
2. Where executable backend plugins are exercised, the cost report shall identify proxy-process, managed connector-process, and aggregate process-tree resource use separately so that moving work across the IPC boundary cannot be reported as a proxy-wide reduction.
3. At every attempted 1k/5k/10k tier, the system shall report the scaling slope and frozen sourced gates for the applicable fixed and rate-driven costs, and shall not certify a tier merely because connections remain open when resource headroom, latency, correctness, durability, or cleanup gates fail.
4. The implementation program shall classify database reads, writes, queries, and transactions on request-start, stream-event, terminal, and cleanup paths as authoritative synchronous work, reusable/versioned read work, best-effort projection, or idempotent terminal work before selecting caching, batching, or asynchronous execution.

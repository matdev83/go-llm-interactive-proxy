...
- [ ] 1.2 Add behavior/lifetime characterization before movement
  ...
  - _Boundary: tests_
  - _Depends: 1.1_
  - _Validation: focused package tests under race where state is concurrent_
...
- [ ] 2.2 Add generation composition and fixed core-port output
  ...
  - Add tests for two overlapping generations, failed candidate compile and last-good isolation.
  - _Requirements: 2.3, 8.3, 8.4, 8.5_
  - _Boundary: standard-distribution composition / generation_
  - _Depends: 2.1_
  - _Validation: featurehost tests; existing generation pin/reload tests_

- [ ] 2.3 Integrate one featurehost handle into `ProcessServices`
  - Add exactly one `StandardFeatures *featurehost.Runtime` (final spelling may follow repository style) to `ProcessServices`.
  - Construct it after the generic dependencies it borrows are ready and before any generation that consumes it can be published.
  - Register exactly one featurehost close with the process owner in the correct dependency order.
  - Do not remove existing per-feature fields yet; this task establishes the parallel structural seam for one migration wave only.
  ...
  - _Depends: 2.2_
  - _Validation: `go test -race ./internal/infra/runtimebundle/... ./internal/standardplugins/featurehost/...`; process shutdown tests_
...
- [ ] 2.4 Route simple predecessor compose adapters through featurehost
  ...
  - _Depends: 2.3_
  - _Validation: reasoning/secretguard parity tests; `git grep` proving runtimebundle does not call dedicated feature compose packages_
...
- [ ] 3.3 Move process ownership under featurehost and delete generic fields/package
  - Featurehost constructs/retains the process-shared coordinator and parent adapter.
  - Remove `BranchCoordinator`, compaction-continuity parent-port and equivalent concrete fields from `ProcessServices`/generic build inputs.
  ...
  - If coordinator gained a real Close/worker after baseline, STOP and update lifecycle design before wiring cleanup.
  - _Requirements: 3.1, 3.5, 3.6, 8.2_
  - _Boundary: featurehost / runtimebundle cleanup / archtest_
  - _Depends: 3.2_
  - _Validation: full compaction tests, generation reload tests, archtest_
...
- [ ] 4.3 Move construction to featurehost and remove the mixed old core package
  ...
  - _Boundary: featurehost / core / infra_
  - _Depends: 4.2_
  - _Validation: runtime conversation-view integration tests; archtest; DB parity suites_
...
- [ ] 5.2 Move prompt/memo/shape/sanitize implementation to the interleaved feature
  ...
  - Feature package imports public canonical/SDK contracts only.
  - _Requirements: 5.1, 5.2, 11.4_
  - _Boundary: feature implementation_
  - _Depends: 5.1_
  - _Validation: moved pure tests; prompt/memo/sanitize fuzz where existing_

- [ ] 5.3 Introduce and bind the narrow `InterleavedProcessor` consumer port
  - Add the smallest runtime-owned interface matching the final methods actually needed by current interleaved stream orchestration. Start from design's `BeginTurn`/turn object shape and delete any method not used by existing orchestration.
  - Featurehost creates a generation-bound processor from feature config and supplies it to executor construction.
  - Nil processor means feature disabled and must preserve current disabled behavior.
  - Core must not import the feature package or its config type.
  - _Requirements: 5.2, 5.3, 8.3_
  - _Boundary: core consumer port / featurehost_
  - _Depends: 5.2_
  - _Validation: runtime interleaved tests hidden/visible/cancellation/failure; import archtest_
...
- [ ] 5.4 Delete `internal/core/interleavedthinking` and ratchet the split
  ...
  - _Depends: 5.3_
  - _Validation: `go test ./internal/core/routing/... ./internal/core/runtime/... ./internal/plugins/features/interleavedthinking/...`; archtest_
...
- [ ] 6.3 Remove keep-warm fields/package from generic process/core
  - Remove `KeepwarmPolicy`, `KeepwarmRegistry` and equivalent concrete fields from `ProcessServices`/executor config.
  ...
  - _Boundary: runtimebundle cleanup / archtest_
  - _Depends: 6.2_
  - _Validation: process shutdown/reload tests under race; archtest_
...
- [ ] 7.1 Move the bounded actor policy store to standard feature infrastructure
  ...
  - _Depends: 6.3_
  - _Validation: moved store tests under race_
...
- [ ] 7.2 Replace core mutable policy dependency with effective snapshot reader
  ...
  - Featurehost owns the mutable store and implements/binds the reader.
  ...
  - _Depends: 7.1_
  - _Validation: request admission tests; HTTP/admin policy tests; terminal provider conflict/chokepoint tests_

- [ ] 7.3 Delete core terminal policy package and per-feature process field
  - Remove `TerminalDecisionPolicy` concrete field from `ProcessServices` and any direct concrete type in executor build inputs.
  - Delete `internal/core/terminaldecisionpolicy`; ratchet absence.
  ...
  - _Boundary: runtimebundle/core cleanup / archtest_
  - _Depends: 7.2_
  - _Validation: terminal-decision full tests; archtest_
...
- [ ] 10.1 Re-run consumer analysis after all known migrations
  ...
  - _Depends: 9.4_
  - _Validation: import graph scan / `go list`; updated ownership manifest_
...
- [ ] 10.4 Remove all per-feature fields from generic process/executor composition
  ...
  - _Boundary: core/runtimebundle cleanup / archtest_
  - _Depends: 10.3_
  - _Validation: runtimebundle/runtime tests; archtest_
...
- [ ] 12.1 Regenerate final ownership census and require zero deferred row
  ...
  - _Boundary: architecture evidence_
  - _Depends: 11.6_
  - _Validation: ownership manifest/census tests; import/structure scans; `make arch-report`_

- [ ] 12.2 Run independent architecture review against requirements/design
  - Reviewer must explicitly check: kernel authority preservation; no feature semantics in core; featurehost not DI/service locator; process/generation cleanup singularity; public host binding safety; config single authority; conversation/interleaved split correctness; no request-hot-path lookup; no output/retry semantic drift.
...
1. **A — prerequisite/census/characterization + featurehost foundation** (Tasks 0–2).
...
4. **D — interleaved-thinking split** (Task 5; likely multiple PRs due routing/state/persistence touch surface).
5. **E — keep-warm extraction** (Task 6).
6. **F — terminal-policy extraction** (Task 7).

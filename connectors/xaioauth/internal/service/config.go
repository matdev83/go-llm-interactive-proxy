...
func ParseConfigYAML(raw []byte) (Config, error) {
	...
	if len(raw) > 0 {
		...
		if err := yaml.Unmarshal(raw, &rawMap); err != nil {
			return Config{}, fmt.Errorf("xai-oauth: config: %w", err)
		}
		for k := range rawMap {
			...
			if lower == "api_key" || lower == "apikey" || lower == "token" ||
				...
				return Config{}, fmt.Errorf("xai-oauth: literal secrets in configuration YAML are forbidden; supply credentials via ConfigureRequest.Secrets or oauth_token_file")
			}
		}
...
func (c Config) HTTPClient() (*http.Client, error) {
	...
	if c.HTTPTimeout != "" {
		...
		if err != nil {
			return nil, fmt.Errorf("xai-oauth: invalid http_timeout %q: %w", c.HTTPTimeout, err)
		}
		timeout = d
	}

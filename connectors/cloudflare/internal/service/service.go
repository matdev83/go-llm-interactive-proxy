...
func (s *Service) Configure(_ context.Context, req backendplugin.ConfigureRequest) (backendplugin.ConfiguredInstance, error) {
	if req.FactoryKind != "" && req.FactoryKind != FactoryKind {
		return nil, fmt.Errorf("cloudflare: unexpected factory kind %q", req.FactoryKind)
	}
	...
	token := strings.TrimSpace(string(req.Secrets.Values["api_token"]))
	if token == "" {
		token = strings.TrimSpace(string(req.Secrets.Values["api_key"]))
	}
	if token == "" {
		return nil, fmt.Errorf("cloudflare: api_token (or api_key) is required in secrets")
	}

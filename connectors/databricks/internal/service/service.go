...
func (s *Service) Configure(_ context.Context, req backendplugin.ConfigureRequest) (backendplugin.ConfiguredInstance, error) {
	if req.FactoryKind != "" && req.FactoryKind != FactoryKind {
		return nil, fmt.Errorf("databricks-ai: unexpected factory kind %q", req.FactoryKind)
	}
	...
	if cfg.Host == "" {
		return nil, fmt.Errorf("databricks-ai: host is required")
	}
	...
	if token == "" {
		return nil, fmt.Errorf("databricks-ai: token (or api_key) is required in secrets")
	}
...
func (i *instance) Execute(stream backendplugin.ExecuteStream) error {
	...
	if i.cfg.ServingEndpoint != "" {
		defaultModel = i.cfg.ServingEndpoint
	}
	return openaicompat.ForwardExecute(stream, openaicompat.ExecuteOpts{
		...
		ResolveModel: func(inv backendplugin.Invocation, call lipapi.Call) string {
			m := resolveModel(i.kind, inv, call)
			if m == "" && i.cfg.ServingEndpoint != "" {
				return i.cfg.ServingEndpoint
			}
			return m
		},

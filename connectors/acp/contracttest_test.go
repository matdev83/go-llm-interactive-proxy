...
func TestSupportedContractTCK(t *testing.T) {
	...
	result := contracttest.Run(t, contracttest.Config{
		PluginID: "acp", Version: "test", Timeout: 10 * time.Second,
		FactoryKind: service.FactoryKind, ConfigYAML: []byte("base_url: " + srv.URL + "\n"),
		Start:     func(context.Context) (backendplugin.Service, func(), error) { return service.New(), nil, nil },
		StartHost: bufconnHost(service.FactoryKind, []byte("base_url: "+srv.URL+"\n"), backendplugin.SecretBundle{}),
	})

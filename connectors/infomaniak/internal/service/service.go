...
func (s *Service) Configure(_ context.Context, req backendplugin.ConfigureRequest) (backendplugin.ConfiguredInstance, error) {
	if req.FactoryKind != "" && req.FactoryKind != FactoryKind {
		return nil, fmt.Errorf("infomaniak-ai: unexpected factory kind %q", req.FactoryKind)
	}
	...
	if cfg.ProductID == "" {
		return nil, fmt.Errorf("infomaniak-ai: product_id is required")
	}
	...
	if token == "" {
		return nil, fmt.Errorf("infomaniak-ai: api_key (or token) is required in secrets")
	}

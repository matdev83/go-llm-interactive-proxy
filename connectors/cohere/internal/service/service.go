...
func (s *Service) Configure(ctx context.Context, req backendplugin.ConfigureRequest) (backendplugin.ConfiguredInstance, error) {
	if req.FactoryKind != "" && req.FactoryKind != FactoryKind {
		return nil, fmt.Errorf("cohere: unexpected factory kind %q", req.FactoryKind)
	}
	...
	if s.tokenProvider != nil {
		...
		if err != nil {
			return nil, fmt.Errorf("cohere: token provider: %w", err)
		}
		tp = factoryTP
	} else {
		return nil, fmt.Errorf("cohere: token provider not configured (use NewProduction or provide TokenProvider/TokenProviderFactory)")
	}
	...
	return backendplugin.ForwardExecute(stream, func(ctx context.Context, inv backendplugin.Invocation, call lipapi.Call) (lipapi.ManagedEventStream, error) {
		...
		if m == "" || m == FactoryKind {
			if inv.NativeModelID != "" && inv.NativeModelID != FactoryKind {
				m = inv.NativeModelID
			} else if i.cfg.DefaultModel() != "" {
				m = i.cfg.DefaultModel()
			} else {
				return nil, fmt.Errorf("cohere: model is required (set model in config or provide model in request)")
			}
		} else {

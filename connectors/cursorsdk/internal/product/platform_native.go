...
func defaultBridgeBinPath(root string) string {
	...
	// Prefer an explicit connector-root layout (…/bridge-node/bin/…).
	direct := filepath.Join(root, "bridge-node", "bin", "lip-cursor-sdk-bridge.js")
	if _, err := os.Stat(direct); err == nil {
		return direct
	}
	// platform_native.go lives under internal/product/; climb to the connector root.
	return filepath.Clean(filepath.Join(root, "..", "..", "bridge-node", "bin", "lip-cursor-sdk-bridge.js"))
}

...
3. Execute the full spec pipeline. Invoke each sibling skill by name. If the host cannot invoke skills directly, locate its `SKILL.md` beside this skill under the same `skills/` catalog root and read it completely:
   a. Initialize with `kiro-spec-init`, then create spec.json and requirements.md
   b. Generate requirements with `kiro-spec-requirements`, then follow its steps
   c. Generate design with `kiro-spec-design`, then follow its steps
   d. Generate tasks with `kiro-spec-tasks`, then follow its steps
